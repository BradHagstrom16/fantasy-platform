...
**Engineering backlog (known-but-unfixed work):** `docs/engineering-backlog-2026-07-21.md` — the `tests/conftest.py` extraction that would shrink every future framework migration, and the Python 3.14 pass. (Deploy-script hardening, the Flask-Limiter `memory://` multi-worker bug, the CF-edge-IP rate-limit-key gap (2.5, realip fix, PR #129), AND the `preset-all` timer hazard (2.4, ADR-044) are ✅ shipped — see the doc.) Check it before starting infrastructure work, and **re-verify any number it quotes** — two items inherited from an earlier audit turned out stale, one by ~40%, and 2.4's own counts were ~26% low by the time it shipped.

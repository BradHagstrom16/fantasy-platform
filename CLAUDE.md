...
**Production:** Live at `cccfantasy.com`. CCC design system shipped at tag `impeccable-v1`; every WC tab body sits on the Casual-Light pattern. See `PRODUCT.md` + `DESIGN.md` (repo root) for the product/design spine; per-game specialization lives in `games/<slug>/DESIGN.md` (currently `games/worldcup/DESIGN.md`). Any UI work invokes the `impeccable` skill (v3.5.0; the stock loader `context.mjs` reads only the top-level `PRODUCT.md`/`DESIGN.md`). **The stock loader does NOT auto-discover per-game files, so this is a hard rule: when working any UI surface under `games/<slug>/`, you MUST read `games/<slug>/DESIGN.md` alongside the top-level `DESIGN.md` before producing design output** — top-level is authoritative for cross-game/platform-foundation concerns; the per-game file owns that game's palette/accent-rank/register/named primitives (convention recorded in `docs/impeccable-loader-customization.md`). Keep impeccable current **only via `/update-plugins`** (it runs `~/.claude/scripts/impeccable-skills-update.sh`, which updates the global install safely); never run `npx impeccable skills update` from a repo root — it drops a stray project-local copy instead of updating the global install (a guardrail hook in `~/.claude/settings.json` blocks that). Two ship-as-is backlogs predate new polish work: `docs/superpowers/specs/2026-05-12-impeccable-design-improvement-scorecard.md` (22 deferred items) and `docs/superpowers/specs/2026-05-13-worldcup-tab-unification-scorecard.md` (per-tab finals + WC-specific debt) — check both before adding new polish.

**Architecture: lounge vs rooms.** Platform `/` is the **club lounge** — dark CCC purple+gold atmosphere, dominated by whichever single game is currently live. Each game has its own **room** with specialized identity (WC: bone substrate + navy hero + WC red accents; future CFB/Golf each their own). The lounge is *always* single-game-flavored by the active game; cross-game lounge work only begins when ≥2 games run simultaneously. **Substrate distinction between dark lounge and game-specific body is by-design architectural separation, not whiplash.** The lounge stays WC-flavored until WC closes and CFB Survivor launches (~Aug 2026); at that point the four lounge state-shells in `core/main/templates/main/_home_*.html` will need to generalize off WC-specific concepts. Don't try to converge substrates; small handoff polish (e.g., navy bridge band between dark navbar and bone WC body) is acceptable polish-tier work.
...
- **WC body sits on the Casual-Light pattern; `.wc-champion-banner` is the only dark surface left:** Every WC tab body (HUB / ROSTER / BOARD / SCHEDULE / STATS / RULES) uses white `.card` / `.wc-stat-card` on the bone page substrate. The dark navy `.page-hero.wc-hero-grad` (top of each tab) stays as WC's signature identity moment; the dedicated `.wc-champion-banner` primitive (rendered only on the post-tournament champion banner in the **WC-room** `games/worldcup/templates/worldcup/_home_post.html` when a winner is declared, or as the defensive "Tournament Complete" fallback) is the only remaining body-area dark surface. **Two parallel `_home_<state>.html` trees exist** — the WC room under `games/worldcup/templates/worldcup/` and the **lounge** under `core/main/templates/main/`; a grep for `_home_post.html` hits both. The lounge's post-state champion banner is a *distinct* primitive, `.champion-banner` (via `_champion_banner.html`, with `.champion-flag`/`.champion-name`) — **not** `.wc-champion-banner`. The prior generic `.card.wc-card` substrate retired in WC tab unification P5 — when adding new WC surfaces, default to the light pattern; reach for `.wc-champion-banner` only for the existing ceremonial slot. Foreground-color carve-outs that need a bone lift on the banner scope as `.wc-champion-banner .<surface>` (currently `.text-muted` and `.wc-eyebrow:not(.wc-eyebrow-red):not(.wc-eyebrow-gold)`).
...
- **Navbar wordmark + auth bust:** the navbar brand pairs the head mark with the designer's `wordmark-bone.svg` (`<img class="brand-wordmark d-none d-md-inline">` — head-only below md; the `<a class="navbar-brand">` carries `aria-label="Corrupt Commish Club"` so the link keeps an accessible name when the wordmark is hidden). The auth desktop brand panel leads with `mascot-bust.svg` via the shared `core/auth/templates/auth/_brand_logo.html` partial (included by login/register/forgot/reset; each keeps its own page-specific headline/sub/games). Designed wordmark kit lives at `static/img/logo/wordmark-{bone,gold,purple}.svg`; the brand-kit lockups + one-color heads stay unwired in the gitignored `CCC-final/`. Locked by `tests/test_logo_assets.py` + `tests/test_asset_versioning.py` (the auth-panel brand-image versioning lock tracks `mascot-bust.svg`, not the now-unused `icon.svg`).
...
- **Authenticated responses are `Cache-Control: private, no-store`:** an `@app.after_request` hook in `app.py` stamps this on every response when `current_user.is_authenticated` (static endpoint excepted). This is a **security invariant** — do NOT remove it to "improve caching." A shared cache that ignores `Vary: Cookie` (e.g. a Cloudflare "Cache Everything" rule) could otherwise store one logged-in user's rendered page and serve it to another. Public/anonymous responses are intentionally left cacheable so the CDN can still front the marketing/public surfaces; never blanket `no-store` onto anonymous responses. Locked by `tests/test_response_cache_headers.py`.
...
- **`team.best_finish` labels:** `games/worldcup/services/stage.best_finish_label(code)` is the SSoT mapping canonical `WorldCupTeam.best_finish` codes to display strings ("Champion", "Round of 16", "Group Stage", etc.) — a **different value space** from `stage_label` (it drops `final`/`third_place` and adds the podium codes `3rd`/`runner_up`/`champion`). **BOTH** post-state roster recaps consume it — the lounge (`core/main/home_context`) and the WC room (`games/worldcup/services/home_context`) — so they can't diverge (they did: the lounge shipped raw codes + a banned `or 'Group'` fallback, audit finding B1). Two rules the function enforces: (1) empty/`None` best_finish ⇒ `'Round of 32'` — a team that advanced from its group but won no knockout match *reached the R32 and lost*, which is **distinct** from `'group'` (eliminated in the group stage) ⇒ `'Group Stage'`; collapsing both to "Group" mislabeled a group winner as a group-stage exit (finding F1). (2) Unknown codes fall back to the **raw code**, never `'Group'`, so a new scoring code surfaces as the bug it is. Locked by `tests/test_worldcup_stage.py`. Don't reintroduce a per-builder label map.
...
- **Static asset cache-busting:** Every `<link>`/`<script>`/brand-`<img>` referencing `/static/*` in templates appends `?v={{ asset_version }}` (git short SHA, resolved at app boot in `core/context.py::_compute_asset_version`). Required because nginx serves `/static/` with `expires 30d; "public, immutable"` — without the cache-bust, Cloudflare's edge swallows post-deploy changes for up to 30 days (PR #18 fallout, fixed in PR #19). **This now includes brand images** (favicon `<link>`s, navbar mark, footer `seal-color.svg`, and the reset-email `seal_url`): the prior "images skip the param; rename the file to cache-bust if ever swapped" convention was disproven by PR #48, which swapped the logo SVG/PNG/ICO files *in place* (same filenames) — the unversioned URLs stayed frozen behind the immutable edge for 30 days even though the new bytes were on the origin. Email seal is versioned via `?v={{ asset_version }}` in `reset_password_html.j2` (the `render_template` call is request-bound, so the context processor injects `asset_version`). Locked by `tests/test_asset_versioning.py` (`test_rendered_base_brand_images_are_versioned`, `test_reset_email_seal_is_versioned`). Debug recipe if you ever see post-deploy stale assets on cccfantasy.com: `curl -sI https://cccfantasy.com/static/css/style.css | grep -iE 'cf-cache-status|age'` — long `age` + `HIT` means an unversioned link slipped through; fix the template, don't reach for the CF purge button. To distinguish a stale edge from a failed deploy, hash the origin bytes past Cloudflare with a throwaway query key: `curl -s "https://cccfantasy.com/static/img/logo/favicon.svg?cb=$(date +%s)" | shasum -a 256` vs `git show HEAD:static/img/logo/favicon.svg | shasum -a 256`.

...
- **`.card.wc-card` is a dark navy surface:** `background: rgba(0, 17, 46, .8)`. When wrapping content originally designed for light surfaces (e.g., `.leaderboard-table`), Bootstrap's default `<td>` white bg masks the navy and contrast holds. But surfaces with no inner light layer — `<thead th>`, mobile `.leaderboard-card` content, translucent-overlay rows like `.row-current-user` — read dark-on-dark and need explicit light foreground colors scoped to `.card.wc-card .<surface>`. Don't broadcast `tbody td { color: light }` — that breaks the masked-by-Bootstrap rows. Pattern lock: see `static/css/style.css` "Plan 3 follow-up: legibility fixes for the dark .card.wc-card surface".
- **Bootstrap `.text-muted` override is global, not local:** Don't add new `!important` rules to override `.text-muted` color on bone substrates. Bootstrap 5.3 resolves `.text-muted` via `var(--bs-secondary-color)`; the canonical CCC override is the `:root { --bs-secondary-color: var(--text-secondary) }` redirect in `style.css` (~line 7172, S6.1.1 PI-2) which lifts every bone-canvas instance from `#6c757d` (~3.5:1 on bone, sub-AA) to `--text-secondary` (`#5A5470`, ~6.9:1) without specificity wars. Dark substrates still need their own scoped `!important` rules — `.card.wc-card .text-muted` (style.css :6728) and `.page-hero.wc-hero-grad .hero-subhead.text-muted` (:6670) — because the substrate flip changes the contrast equation. New dark surfaces adopting `.text-muted` need their own scoped lift, not another `:root` override.

...
- `games/cfb/` — CFB Survivor Pool — **active focus**; registry `open` + featured since the 2026-08-11 changeover (season starts Thu Sep 3); co-headlines the lounge with The Docket since 2026-08-18 (ADR-049). Self-serve joining closes at the shared enrollment deadline, Sat Sep 5 11:00 AM CT (`join_window_open` in `games/cfb/services/lounge.py`, ADR-050). **The lounge flips pre→live at `SEASON_LIVE_UTC`** (Tue Sep 1 2026 6:00 AM CT, same module) — a time gate equality-locked to the docket's Week-1 boundary, because an activated week alone never means live: preview imports exist before the season by design (2026-08-19 ruling). Dark-first room + admin cluster shipped; design doctrine in `games/cfb/DESIGN.md`.
- `games/docket/` — The Docket (NFL+CFB weekly pick'em) — **active focus**; registry `open` + **co-headlines the lounge with Survivor since 2026-08-18** (ADR-049; lounge set = `games/docket/services/lounge.py` — pure-time state resolver, never the DB — plus `games/docket/templates/docket/lounge/`; self-serve joining closes at the shared Sep 5 deadline, ADR-050). Light court-paper room; design doctrine in `games/docket/DESIGN.md`; concept brief in `docs/2026-08-11-nfl-cfb-pickem-office-hours-kickoff.md`; binding rulings (Premises, Session Rulings, Grading Clarifications, Eng Review Addendum, RULING OVERRIDE) in `docs/2026-08-11-docket-binding-rulings.md`. **Cite the suffixed form, always** — there are two D-namespaces from two sittings and they collide at D5–D11 with entirely different content: `D5-session` is the autopick package, `D5-eng` is the shared odds client. The fuller session artifact this was migrated from stays a local gstack file; everything binding is in-repo. Pick sheet + join shipped (T7); sync/recalc CLI + deadline pass shipped (T8); minimal admin shipped (T9); timers + D24-eng reminders shipped (T11a); the season ledger + rules page shipped (T10); the D13-eng systemd preset (T11b, backlog 2.4) shipped as ADR-044; the lounge second-bill strip shipped (T13) and went **dormant** at the 2026-08-18 multi-featured rework: `games.registry.second_bill_games(user)` now returns open games that aren't lounge headliners (empty under the dual-featured registry; machinery + `lounge_cadence` retained for a future open-unfeatured game, reappearance-locked in `tests/test_docket_lounge_strip.py`). The partial stays registry-generic and static by ruling (D21-eng) whenever it renders. **The season pass consumes `WeekRollup`, never `WeekGrade`** (ADR-045): a `PlayerWeekGrade` requires exactly eight `SlotGrade`s and the persisted rollup stores no slot trace, so do not try to rebuild a `WeekGrade` from the DB — `grading/season.week_rollup()` projects the engine path onto the same type `services/season_pass.py` builds from rows. That module reads `docket_week` + `docket_week_result` and nothing else, which is D14-eng as a query plan; the parity suite replays all six season fixtures through it. **`DocketWeekResult.is_dropped` has no writer by decision** (ADR-046) — the drop is derived on every read, so do not add one. **`DocketWeek.default_error_tenths` is written by `run_grading_pass`** (ADR-047) and is what charges a week to members absent from it; a graded week with a NULL there is refused loudly rather than skipped. **The grading population for a CLOSED week is `roster_user_ids_as_of(week.deadline_at)`, never the live roster** (ADR-048) — grading a past week against today's roster hands a later joiner a full autopick package worth real points (verified: 8.0 of 9.0), and only if someone ran `recalc`. **D24-eng reminders de-dup on the sent flag (`DocketWeek.last_reminder_tier`), never on cadence** — that is why `docket-remind.timer` can run hourly; do not reproduce CFB's cadence-dependent shape. The blueprint lives in `games/docket/blueprint.py`, not the package `__init__` — the pure grading package's flask-free import graph is a locked contract (D9-eng); it registers **two** route modules, `routes.py` (the player's room) and `admin_routes.py` (the clerk's office, which also owns `docket_admin_required`). **Grading readiness has two distinct shapes**: `grading_pass.WeekNotReady` (a `ValueError` subclass) means "resolves by waiting" — the deadline pass hasn't stamped `kickoff_at_deadline`, or no tiebreaker is designated yet — and `try_grade_week` reports it as `not_ready`; every other `ValueError` is corrupt data and propagates, so a timer fails loudly instead of exiting 0 forever. Don't widen that catch. **Pick provenance is two columns, not one**: `is_autopick` marks a side the deadline pass filed, `is_auto_best` marks a designation it assigned — auto-designation evaluates the final 8-slot set, so the double can land on a pick the player made themselves, where `is_autopick` stays False (`is_auto_best` implies `is_best`, CHECK-enforced). Both surface on the sheet through the dashed treatment; the rail states the reason once. **Admin rulings all live in `games/docket/services/admin_ops.py`** — designation (re-designating clears the week's predictions and emails the roster), No Contest (auto-recalcs the week, reversible), and D18-eng line correction (pre-deadline AND pre-kickoff, reason required, audit row in `docket_line_correction`, affected picks explicitly re-snapshotted, pickers emailed). `flask docket set-tiebreaker` is the fallback and delegates to the same service, so CLI and UI can't drift. Prod docket tables stay empty until the deliberate Week-1 import at the real line freeze (Tue Sep 1); note **Week 1 is CFB-only** (the NFL opener is Sep 10, in Week 2), so an empty NFL half in a Week-1 sync is expected, not a failure.
...
**Engineering backlog: none.** The 2026-07-21 backlog was burned down (PRs #169–#173) and the doc deleted 2026-08-21; `git log --follow docs/engineering-backlog-2026-07-21.md` is the archival record. Deferred by date, deliberately — not backlog: the **Python 3.14 pass** (December 2026 quiet window; the conftest prerequisite shipped in #173; bump the droplet venv, `.github/workflows/test.yml` `python-version`, and `ruff.toml` `target-version` together, and re-verify cp314 wheels + the deadsnakes PPA first — nothing enforces those couplings) and the **golf `lazy='dynamic'`/`backref` cleanup** (rides Golf Phase U, ~Jan 2027). Standing policies are conventions, not backlog: `Model.query` (Code conventions + ADR-039), dependency pins (ADR-037/042). The doc's enduring lesson stays in force: **re-verify any number a doc quotes before acting on it.**
...
**Architecture: lounge vs rooms.** Platform `/` is the **club lounge** — dark CCC purple+gold atmosphere billing every headliner as an equal (**multi-featured since 2026-08-18, ADR-049**: CFB Survivor + The Docket co-headline as The Undercard — two accent-washed panels at a gold seam). Each game has its own **room** with specialized identity (WC: light Casual-Light body; CFB: dark-first midnight room; Docket: light court-paper). **Substrate distinction between the lounge and a game body is by-design architectural separation, not whiplash** — don't try to converge substrates (small handoff polish is fine). The lounge dispatches through the registry's multi-featured seam: `games.registry.lounge_games()` returns every featured-open entry carrying **both** lounge callables (`lounge_state` resolver + `lounge_context` builder — an entry missing either never seats), GAMES order = billing order; `lounge_game()` survives as "first billing" for the legacy consumers. `core/main/routes.py` resolves each headliner's state, filters visibility per viewer (`visible_headliners` — the ADR-050 enrollment window: post-deadline, a member stops seeing games they did not join), aggregates the shell state (**live > post > pre**), and renders the composite shell (`main/_lounge_out.html` for anonymous, `main/_lounge_composite.html` authed), which includes one per-game panel per visible headliner (`'<slug>/lounge/_panel_<state>.html'`, conversion cards via `'_conv_card.html'`; each panel reads the loop-local `h` and sets `g = h.ctx`). **2026-08-18 design-review amendments (ADR-051):** panel headers bill `entry.lounge_label` full game names ('CFB Survivor', 'The Docket · Pick ’Em' — never mutate `short_name`, it is string-locked and feeds navbar+tiles); every pre panel opens with the decree seal band, numbered per era (WC No 001, CFB No 002, Docket No 003; the docket's is band-only) plus a matched ◇ `.summons-eyebrow--info` on both games (ADR-052 folded the CFB decree's former `<h2>` into it); the season ledger renders as the shell-owned full-width `main/_lounge_ledger.html` from a registry-generic `ctx['ledger']` lift (cfb's farewell dict carries `endpoint`; the strip is never a panel interior); the composite greet is enrollment-aware (joined-nothing + any window open ⇒ the Welcome/Choose Your Games variant, retiring with the asks per ADR-050; the logged-out bill gets a matching bridge line); and lounge roster counts are floor-gated (`ROSTER_COUNT_FLOOR = 6`, defined in BOTH lounge services and equality-locked like the shared deadline). **The sole exception is the archival page mode**: a SOLE headliner whose entry is `lounge_mode='page'` (worldcup only, ever) renders its full `'_home_<state>.html'` tree byte-identically to the pre-seam dispatch — that branch is what keeps the frozen-WC test nets green; `tests/_registry_helpers.pin_wc_era` is the one sanctioned way to pin that era. `core/main/home_context.build_home_context(user, state, headliners=None)` is the thin core dispatcher: `headliners=None` is the legacy flat single-overlay (page mode + direct callers); a resolved list builds **namespaced `Headliner` records** (entry, per-game state, ctx, tree, enrollment, join_open) so two games can never collide on a context key, with registry-generic keys assigned after the builds (unclobberable, hijack-locked); it must never import a game module directly (locked in `tests/test_registry_seam.py`). **The logged-out surface renders one conversion card per headliner** — each game's own `_conv_card.html`, so no card can wear another game's copy; the rendered-page count lock lives in `tests/test_docket_lounge_strip.py`, now derived from `lounge_games()`. Lounge accents are signature-only via the `--lounge-*` tokens + `--hl-*` mapping (never room `--game-*` vars or room classes; `tests/test_lounge_accent_firewall.py`). **Lounge CTA parity (ADR-052, 2026-08-19):** every action button in both headliner panels is a solid `.hl-cta` in the game's own accent (crimson/garnet) in every state — join, enter, choose, open your sheet; gold is lounge chrome only (the seam, the seal band, the ◇/◈ glyphs), so no `.cta-seal` fills a panel button (retired from CFB's decree + summons; it survives only in the frozen WC archived lounge) and no bone `.cta-outline` either. The one accent-outline (`.hl-cta--outline`) is CFB's HELD "Review Pick"; the dormant second-bill strip keeps its registry-generic gold `.cta-outline`. Locked by `tests/test_lounge_cta_parity.py`. The second-bill strip is **dormant machinery** (empty under the dual-featured registry, retained for a future open-unfeatured game). The WC lounge set remains archived behind the seam — `games/worldcup/services/lounge.py` + `games/worldcup/templates/worldcup/lounge/_*.html`, moved intact for archive/revival.
...
FLASK_APP=app.py venv/bin/flask docket sync --mode lines     # Gap-fill still-empty markets + D19-eng kickoff refresh; warns on a bad tiebreaker designation (Tue-Fri)
...
# `--scheduled` is the TIMER-ONLY flag: it turns exactly two states — out of season, and week-not-imported-yet —
# into a logged exit 0, and softens nothing else (a missing designation at the deadline still exits 1). Typed by
# hand, without it, those same states refuse loudly. The docket season is 19 of 52 weeks, so without this flag
# four of the five timers would report failure on every firing for most of the year and a red docket timer would
# stop meaning anything. Every unit's ExecStart carries it, locked by tests/test_docket_timers.py.
...
- **Navbar lockup + solo-game hoist + auth bust:** the navbar brand pairs the head mark with `wordmark-bone.svg` at **every** width (CSS hides the wordmark only ≤350px; the `<a class="navbar-brand">` carries `aria-label="Corrupt Commish Club"` for the accessible name there). The switcher carries **active** joined games only: `core/context.py` splits `joined_games(user)` into `nav_games` (status != `'completed'`) and `nav_archived` (completed — rendered as the member-gated "The Archive" section of the account dropdown; `joined_games()` itself still returns completed games, the registry archive contract in `tests/test_registry_seam.py` forbids filtering there). When a user's ACTIVE joined-game count == 1, that game's link hoists out of the hamburger into the bar itself (`.navbar-solo-game`, `d-lg-none`, right of brand) and its collapse copy hides below lg — exactly one rendered link per breakpoint; 0 or 2+ active joined games fall back to collapse-only (a WC-only member gets no hoist, only the Archive entry). The auth desktop brand panel leads with `mascot-bust.svg` via the shared `core/auth/templates/auth/_brand_logo.html` partial. Wordmark kit: `static/img/logo/wordmark-{bone,gold,purple}.svg`; unwired brand-kit assets stay in the gitignored `CCC-final/`. Locks: hoist + wordmark gating in `tests/test_navbar_solo_game.py`; asset existence + auth/footer surfaces in `tests/test_logo_assets.py`; versioned URLs + accessible name in `tests/test_asset_versioning.py`.
...
- **Avatars:** All game standings must display `user.get_avatar()` inline before the player display name (required integration point for every game blueprint). `User.avatar_emoji` is nullable String(4); the default is the football (`User.DEFAULT_AVATAR = '\U0001F3C8'`, applied at render time — there is no stored default, so it also covers legacy accounts that never chose). **Two glyphs are reserved and enforced inside `get_avatar()`** so every call site inherits the rule, not just the picker: the **crown** (`ADMIN_AVATAR`) renders for every `is_admin` user; the **trophy** (`CHAMPION_AVATAR`) renders only for the reigning Survivor champion — `User.REIGNING_CHAMPION_USERNAME` (`'cubbies22'`, the 2025 title; re-point it when the 2026 title resolves), matched through `normalize_identifier`. Precedence: admin crown > champion trophy > stored choice > default; anyone else holding a reserved glyph renders the default. Both are excluded from `AVATAR_CATEGORIES` in `core/auth/routes.py` (the trophy's slot is ⚾) and `profile.html` shows a note instead of the picker to anyone wearing one; the picker's JS deselect default is templated from `data-default-avatar`, never a literal. Always the `'\U…'` escapes in `.py` (`models/user.py`), never a literal char — literal non-BMP emoji can break import as invalid surrogate pairs. Locked by `tests/test_auth_avatar_phone.py`.
...
- **Scheduled-jobs state (as of 2026-08-12):** all `worldcup-*` timers disabled + snapshot cron commented (WC archived); Golf jobs disabled (Golf runs on its separate PythonAnywhere box until migration); CFB timers (`deploy/cfb-*`, five pairs) get installed + enabled at launch — runbook in the transition plan §6F. Docket timers (`deploy/docket-*`, five pairs) ship with T11a; enable `docket-{lines,scores,deadline,remind}` early (they stand down quietly against empty tables and so get real soak time before Week 1), and hold **`docket-setup.timer` back until after the hand-run Tue Sep 1 Week-1 import** — that timer is the one that writes to empty prod tables, and the first write is a deliberate act by ruling. Systemd timers are the canonical scheduling mechanism; the legacy crontab lines stay commented as history.
...
- `deploy.sh` — one-command deploy on the server: `git pull` → `pip install` → `flask db upgrade` → **sync every `deploy/*.service` + `deploy/*.timer` into `/etc/systemd/system/`** → **sync `deploy/*.preset` into `/etc/systemd/system-preset/`** → `daemon-reload` → `systemctl restart` → verify `is-active`. The unit sync (ADR-040, generalized to all units by ADR-041) means **editing a unit file in the repo IS the deploy** — never `sudo cp` one by hand. Each unit is validated with `systemd-analyze verify` before it lands, so a broken unit is refused rather than installed, and gets its own verdict: one bad unit warns without aborting the rest. Units with no `/etc` counterpart are **installed, not skipped** — installing is not enabling, and this is what keeps the CFB timer install from depending on someone remembering a manual step; `systemctl enable` stays a deliberate, separate act. The script exits **non-zero** if any non-fatal step warned or if the service fails to come up — a warning plus "App is live" is not a state it can end in.
- **Preset policy is a separate path, not a unit (ADR-044).** `deploy/10-fantasy-platform.preset` bounds what `systemctl preset-all` can do: `disable worldcup-*.timer` (archived, mails real players, `Persistent=true` so an accidental enable fires immediately), `ignore` for `cfb-*`/`docket-*`/`golf-*` (enablement is hand-managed ops state; `ignore` means preset-all can neither switch them on early nor switch a live one off, so this file needs no flip at any milestone). **`systemd-analyze verify` rejects preset files**, so `sync_preset` gates on a bash shape lint instead: every pattern must start with a non-wildcard character, end in `.timer`, **and** carry a prefix that `deploy/*.timer` actually ships. All three are load-bearing, because a preset in `/etc` outranks the vendor `90-systemd.preset` for everything it matches — `disable *.timer` passes a naive suffix check while matching every timer on the box, and `disable apt-*.timer` passes the suffix *and* wildcard checks while still disabling `apt-daily.timer`. The **`10-` prefix is load-bearing** (preset files sort lexicographically across all preset dirs, first match wins; a bare `fantasy-platform.preset` would sort *after* `90-`). A new game's timers are uncovered until a line is added — `tests/test_systemd_preset.py` fails CI on any `deploy/*.timer` prefix with no rule, which is what stops the exposure growing by inheritance the way it went 4 → 14 → 19. **🚨 `systemctl preset-all --dry-run` IS NOT A DRY RUN** — systemd accepts `--dry-run` and silently ignores it for `preset`/`preset-all` (only verbs like `reboot` honour it), and it writes to stderr so `> file` captures nothing. Run on the droplet 2026-08-13 it really enabled 15 game timers plus `rsync`/`nftables`/`iscsid` and six system timers. **To inspect, use `systemctl list-unit-files 'worldcup-*.timer' 'cfb-*.timer' 'docket-*.timer' 'golf-*.timer' --no-pager`** (no sudo, no writes) **and read BOTH columns** — they answer different questions and the incident is where they diverge. `PRESET` is what policy says (`disabled` for `worldcup-*`, `ignored` for the active three); `STATE` is what is actually enabled (`enabled` for `docket-{lines,scores,deadline,remind}` only, everything else `disabled` — including `docket-setup.timer` until after the Tue Sep 1 import). Mid-incident `PRESET` read `ignored` while `STATE` read `enabled` on 15 timers, so a PRESET-only check reports all-clear with the hazard live. Recovery from an accidental run is `systemctl disable --now` naming each unit — `ignore` cannot repair it, since it preserves whatever state it finds, wrong ones included.

...
**Production:** Live at `cccfantasy.com`. CCC design system shipped at tag `impeccable-v1`. Any UI work invokes the `impeccable` skill. Its loader resolves **exactly one** `DESIGN.md` — with `--target` it walks up to the nearest dir holding `PRODUCT.md` *or* `DESIGN.md` and resolves each doc there, falling back to the root only for what that dir lacks; so `--target games/<slug>/…` loads `games/<slug>/DESIGN.md` + the root `PRODUCT.md` and **drops** the top-level `DESIGN.md`; no `--target` loads only the top-level pair. **Hard rule: when working any UI surface under `games/<slug>/`, read `games/<slug>/DESIGN.md` alongside the top-level `DESIGN.md` before producing design output** (top-level owns cross-game/platform concerns; the per-game file owns that game's palette/accent-rank/register/primitives). Impeccable is **one user-level install**; update it via **`/update-plugins`**, which also bumps the CLI and syncs the four `impeccable-*` Claude subagents. A bare `npx impeccable update` is safe from anywhere since the v4 CLI (it updates the install that exists and never creates one) but leaves the subagents stale; a bare `npx impeccable install` in a repo can create a project copy, so a guardrail hook blocks it unless it passes `--user`. `.impeccable/config.json` (detector exceptions + hook settings, each exception with its `--reason`), the `design.json` sidecar and `games/<slug>/.impeccable/surfaces/` briefs are committed; `config.local.json` and session output stay ignored.
...
FLASK_APP=app.py venv/bin/flask cfb sync --mode spreads     # OPENS the week (ADR-062): works on the lowest unlocked week with games, locks spreads at first fetch (Tuesday, DQ-6), and the first line flips is_active + sends the picks-open letter in the same run; later runs fill gaps only and make no odds call when every line is locked; API errors email the admin. Since ADR-065 step 5 the Tuesday open runs INSIDE the Paper (club-paper.timer Tue 06:15 CT, same opener with announce=False, announced in the club letter); cfb-spreads.timer is Friday-only (tests/test_cfb_timers.py)
...
# Pick reminders are the Club Desk's (`flask club desk`, below): windows T-26h35m..24h25m (day-before) and T-2h35m..25m (final) each span two hourly firings, the second an outage retry; de-duped on CfbWeek.last_reminder_type; the final's copy says the time actually left (tests/test_cfb_reminders.py). There is no `--mode remind` since ADR-065 step 9.
...
# Hand-firing a reminder pass ON THE DROPLET: `sudo systemctl start club-remind.service` (golf: `golf-remind.service`), never
# `flask club desk` / `flask golf … --mode remind` in a shell — systemd merges a manual start with an in-flight timer firing of the
# same oneshot unit, which is what makes the sent-flag race impossible; there is deliberately no lock in code (PR #169).

# Docket CLI (timers ship as deploy/docket-*; the units pass --scheduled, see below)
FLASK_APP=app.py venv/bin/flask docket sync --mode setup     # Create week + import slates + lock first-posted lines + rule-derived tiebreaker (Tue). Against an existing week, gap-fills only — locked lines are NEVER overwritten (runbook in games/docket/cli.py). Since ADR-065 step 5 the Tuesday run is club-paper.timer's (same setup, announced inside the Paper); the docket-setup unit was retired at step 9, so this mode is hand-run only.
...
FLASK_APP=app.py venv/bin/flask docket edge [--week N] [--submit-time ISO] [--top 9] [--projections FILE|-] [--sva-weight 0.25]   # READ-ONLY: rank the week's frozen lines against the market (games/docket/services/edge.py): median point across ten books incl. Pinnacle, juice ignored (it predicts nothing beyond the point, scripts/fit_nfl_margin.py); NFL spreads on the fitted key-number model (services/nfl_margin.py), everything else a normal curve; --projections blends NFL team-score projections (Brad's private Subvertadown capture, piped on stdin so it never lands on the box) at --sva-weight; 2 credits/sport on /odds from DOCKET_EDGE_ODDS_API_KEY when set (else ODDS_API_KEY); needs the DB that holds the frozen lines (prod)
...
FLASK_APP=app.py venv/bin/flask club desk --dry-run [--now ISO] [--anchor cfb,docket] [--ride F,S]   # One firing of the reminder desk: prints per member what would go (anchor letter, rider) and what would latch. SENDS NOTHING, WRITES NOTHING. --now is the ONE clock the run reads (naive = CT); --anchor = games the desk may lead for, --ride = slots (F=cfb/warning, S=cfb/final, D=docket/2h) that may carry a rider. Without --dry-run it sends and latches. THE reminder pass for every game since step 9: unit deploy/club-remind.* (hourly :00 CT, `flask club desk --scheduled --anchor cfb,docket --ride F,S`; the Docket rides Friday's warning and Saturday's final, its Sunday 2h anchors alone; the D24 tiers are 48h/24h/2h ±35m, sent-flag de-duped, no API credits). The legacy passes, both `--mode remind` modes and the cfb-remind/docket-remind/docket-setup units are gone; deploy.sh warns if a retired remind timer is still enabled beside club-remind.timer (tests/test_club_timers.py locks the exact flags; tests/test_club_desk_matrix.py is the spec). Hand-fire ON THE DROPLET with `sudo systemctl start club-remind.service`
FLASK_APP=app.py venv/bin/flask club paper --dry-run [--now ISO]                                   # The Tuesday Paper composed from whatever is open right now (the openers are NOT called under --dry-run): prints per member the sections and subject; sends nothing, writes nothing. Without --dry-run it opens the Docket week first, then Survivor, sends, and latches picks_open_notified per game + record_notified on the Docket's previous week. Unit: deploy/club-paper.* (Tue 06:15 CT, TimeoutStartSec=10m; tests/test_club_timers.py). Hand-fire ON THE DROPLET with `sudo systemctl start club-paper.service`, never the bare command (same reasoning as the reminder passes)
...
# The standalone Docket record pass (docket sync --mode scores) is eligible only from 60 min past the NEXT week's boundary (games/docket/services/record.py RECORD_HANDOFF): Tue 05:15 grades and stands down, the 06:15 Paper carries the record and latches it, Wed 08:00 sends whatever the Paper did not deliver (tests/test_docket_record.py).
...
- **No eyebrow above a heading (ADR-066):** in the CFB and Docket rooms (heroes, admin mastheads, in-page headlines), the Settle the Tab card and every Club Letter. A fact the label carried goes in the heading or the line under it; `Letter` has no `eyebrow` field, so a game letter's headline names the game and week ("The Docket, Week 3: 5-3"). A label over a value, list, paragraph, fold or stamp stays, and so does a desk letter's per-game section head. Out of scope: the lounge's ◇ summons (ADR-052) and frozen WC; platform admin, auth and `/app` are not yet converted (`tests/test_eyebrow_above_heading.py`).
...
- **Emails:** all outbound via `utils/email.py` → `send_platform_email()`; From-name "Corrupt Commish Club"; game content in `games/<game>/services/reminders.py`. Admin/ops alerts stay plain text.
  ...
  - **Admin announcements (`/admin/announce`) are markup, never HTML:** `utils/letter_markup.py` parses the body into Club Letter blocks (every helper in `utils/email_layout.py`); every mistake is a line-numbered error, never a silent drop. Live boards (`[[survivor-board]]`, `[[docket-week week=3]]`, …) live in `games/<slug>/services/announce_blocks.py` and show a COMPLETE Survivor / GRADED Docket week only (the reveal guard). Drafts and the sent archive are `models.content.Announcement`: a sent row is immutable, keeps the mailed copy (`sent_html`/`sent_plain`), and `claim_for_send` sends it once (`tests/test_letter_markup.py`, `tests/test_announce_blocks.py`, `tests/test_announce_drafts.py`).
  - **The Tribune (`core/tribune/`, `/tribune`) is the members-only archive of every sent announcement**, filed under the season week it speaks to (the "Week N" in its headline or subject, else the Docket week math's club week of the send; `services.filed_week`). The send stores TWO more things beside the mailed copy: `sent_page_html` (the same `Letter` through `utils.email_layout.render_letter_page` and `templates/letters/page.j2`, the letter's blocks with no shell, so a live board is frozen as of the send) and `week_number`. Never re-render a sent issue from `body` on read (a row without the page copy prints its `sent_plain`); `flask tribune backfill` is the one-shot for rows sent before those columns existed (idempotent, boards as of the backfill date; a body that no longer parses is skipped and named, the rest commit). The lounge's "Latest letter" line reads `models.content.latest_issue()` only, never the blueprint (hijack lock). `templates/email/` must hold only `letter.j2` (`tests/test_email_letter.py`), which is why the page shell lives under `templates/letters/`. `tests/test_tribune.py`.
...
- **Scheduled jobs:** every `deploy/*.timer` is installed by every deploy (ADR-041) and stays `disabled` until enabled by name. Query the truth: `systemctl list-unit-files 'worldcup-*.timer' 'cfb-*.timer' 'docket-*.timer' 'golf-*.timer' 'scores-*.timer' 'club-*.timer' --no-pager` (no sudo). **Off for good:** `worldcup-*`. **Held until Phase L (~Jan 2027):** all six `golf-*` (free-tier cadence, ~115 API calls/mo — never widen without re-doing that arithmetic). **`club-paper.timer` opens BOTH games on Tuesday** (ADR-065 step 5; replaced `docket-setup.timer` and the Tuesday half of `cfb-spreads.timer`) **and `club-remind.timer` sends every reminder tier of both games** (step 9; replaced `cfb-remind.timer` + `docket-remind.timer`). Both must stay enabled until Survivor's last CFP week in late January, which is after the Docket's season ends on Jan 12 — never disable them when the Docket goes quiet. The three retired units are gone from the repo; `deploy.sh` never removes a unit from the box, so they were `rm`'d by hand at the step-9 cutover, and it warns if a retired remind timer is enabled beside `club-remind.timer`.
...
core/            # auth/ (no URL prefix — /login, /profile; tokens.py), admin/, main/ (lounge), push/ (The Wire), tribune/ (the Club Letter archive), context.py (nav context processor)
...
templates/       # base.html, email/ (letter.j2 only), letters/ (page.j2, the Tribune's page shell), errors/

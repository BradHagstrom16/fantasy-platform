...
**Engineering backlog (known-but-unfixed work):** `docs/engineering-backlog-2026-07-21.md` — deploy-script hardening (incl. the self-modifying-script bug: a change to `deploy.sh` takes effect one deploy *late*), the Flask-Limiter `memory://` multi-worker bug, the `tests/conftest.py` extraction that would shrink every future framework migration, and the Python 3.14 pass. Check it before starting infrastructure work, and **re-verify any number it quotes** — two items inherited from an earlier audit turned out stale, one by ~40%.

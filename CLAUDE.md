...
FLASK_APP=app.py venv/bin/flask worldcup send-group-recap  # Send admin-triggered group-stage recap email (guarded: all 12 groups confirmed; marker-idempotent)
...
- **Time test seam:** `games/worldcup/services/state.now_utc()` is the canonical "now" reader for **all WC application code** — state detection, `_context_*()` builders, routes computing `deadline_passed`, CLI commands deriving "today". It honors `WC_FAKE_NOW` (ISO 8601) when `ENVIRONMENT` is `development`/`testing`, so a faked tournament moment applies consistently across the whole WC surface. Never call `datetime.now()` directly in WC application paths. Exception: SQLAlchemy `default=lambda: datetime.now(UTC)` audit-timestamp lambdas (`created_at`/`updated_at`) record real wall-clock time, not faked time. CFB mirrors the contract: `games/cfb/utils.get_current_time()`/`get_utc_time()` honor `CFB_FAKE_NOW` under the same env gating (naive ISO ⇒ UTC; locked by `tests/test_cfb_time_seam.py`); the `patch.dict` + `ENVIRONMENT` gotcha below applies to `CFB_FAKE_NOW` identically. CFB datetime **columns** are stored naive with a split contract — `deadline`/`start_date`/`game_time` are pool-tz wall clock (read via `make_aware`), `created_at`/`spread_locked_at` are UTC (read via `to_pool_time`) — documented in `games/cfb/models.py`; using `make_aware` on a UTC column shifts it +5/6h (the recap-AUTOPICK mislabel bug).
...
- **Results automation:** `games/worldcup/services/sync.py` (`flask worldcup sync`, football-data.org free tier, 30-min systemd timer) auto-applies completed-match results through `process_match_result` — match data stays SSoT. **Group advancement + the group→R32 transition stay admin-confirmed** via the "Load from API" pre-fill on `/worldcup/admin/advancement` (tiebreaker risk; never auto-written). The **deterministic downstream KO rounds (R16→Final) auto-fill PER SIDE, incrementally** via `games/worldcup/services/bracket.py` (`run_bracket_autofill`, folded into the 30-min `run_scores` + `flask worldcup sync --mode bracket`): each shell **side** fills the moment its feeder match completes (winner, or SF loser for third place) — opponent side stays TBD until decided — mirroring how the API/ESPN resolve sides one at a time (so a group-winner like Canada appears in its R16 slot the same sync its R32 match ends, not after the whole round). `derive_sides` derives each empty side from our own results + the fixed `BRACKET_TOPOLOGY`; `reconcile` cross-checks **by membership** against the API's per-side resolution (`fetch_bracket_proposal(...)['sides']`) — orientation-independent: a derived team is confirmed if the API has it on *either* side, **conflicts only when the API has FULLY resolved that fixture to a pairing excluding it** (CONFLICT → that side blocked, admin email once), and writes from our results alone (flagged spot-check email) when the API hasn't resolved it yet. Writes go through `set_knockout_team_side` (empty-side-only → never clobbers a manual override or the opposite side); confirmed fills are silent (logged + in the digest). **Self-gated via `derive_sides`, NOT `populatable_bracket_stages`** — that whole-round gate (`ko_round_pending`) still drives the admin bulk-populate UI + the advancement reminder, but the auto-fill must process a stage as soon as ONE feeder resolves; don't re-wire `run_bracket_autofill` back onto `populatable_bracket_stages` (it would stall every round until 100% of the feeder round completes). Empty shells only; the manual `/worldcup/admin/bracket/<stage>` pages stay as the override for every KO round. Don't add a parallel results-entry or scoring path. Player daily digest (`flask worldcup send-digest`, `services/notifications.py`) fires 5am CT via `worldcup-digest-player.timer` — one email per player per day, only when picks scored the previous CT calendar day. Specs: `docs/superpowers/specs/2026-06-02-worldcup-results-automation-design.md` + `2026-06-02-worldcup-daily-digest-design.md` + `2026-06-28-worldcup-knockout-bracket-autofill-design.md`.
- **Bulk bracket populate (review-then-confirm):** when a KO feeder round is resolved, `/worldcup/admin/bracket/<stage>` pulls that round's pairings from the API and assigns every shell at once after an admin confirm. `services/sync.py` helpers: `fetch_bracket_proposal(stage)` (read-only proposal — never writes), `populatable_bracket_stages()` (which stages the dashboard offers a CTA for), `all_group_advancement_confirmed()` (the R32 gate). The per-shell `admin/set_knockout.html` form stays the manual override; don't add a parallel bracket-write path.
- **Group-stage recap email (admin-triggered):** `notifications.send_group_stage_recap()` → `/worldcup/admin/send-group-recap` dashboard button + `flask worldcup send-group-recap`. Guarded by `all_group_advancement_confirmed()` (refuses until all 12 groups confirmed); marker file `instance/.wc_group_recap_sent` records the last send for the resend label (visibility, not a hard lock). Mirrors the daily-digest structure (HTML + plain-text, one email per enrolled picks-submitted player).
...
- **Display-layer podium attribution:** podium bonuses (`champion`/`runner_up`/`3rd`) are non-match ScoreEvents (`match_id=None`) by SSoT contract, but every per-match **display** surface attributes them to their deciding match via `scoring.display_points_for_pick_on_match(pick, match)` → `(multiplied pts incl. podium, podium code|None)` / `scoring.podium_bonus_for_match(team_id, match)` → `(code, base pts)|None` — consumers: lounge + hub results strips, daily digest, `team_detail` match log (which keeps the podium bonus as a "Beyond the matches" line item only when its deciding match is absent from the log). New per-match displays use the display helpers; never fold podium into `points_for_pick_on_match` itself (locked: `test_points_for_pick_on_match_third_place_winner_returns_zero`) and never render a won final/bronze match as 0 pts (the 2026-07-19 "NO POINTS" incident). Note the runner-up case: a **lost** final earns points — label it (gold, ceremonial register), don't reuse win/loss framing.
- **Competition rank everywhere:** `routes.leaderboard()` and `services/ranking.compute_rank_neighbors()` both use competition rank — tied scores share a rank, the next distinct score gaps by the size of the tie (`1, 1, 3, 4`, NOT dense `1, 1, 2, 3`). The canonical formula is `rank = 1 + (count of enrollments scoring strictly higher)`. Never reintroduce dense rank; a tied player's leaderboard rank must equal their `/worldcup/leaderboard/<id>` detail rank ("Your Standing" reuses `compute_rank_neighbors` and depends on this parity). The same competition-rank loop is mirrored in `routes.rosters()`, `services/home_context._context_post`, `core/main/home_context` (lounge dossier + top-3 preview), `services/notifications._competition_rank`, and the nightly `cli.snapshot-ranks` capture — keep them in lockstep. Jinja idiom for top-N preview tables: `{% set ns = namespace(rank=0, prev_score=None) %}` then `{% if e.total_score != ns.prev_score %}{% set ns.rank = loop.index %}{% endif %}` (`loop.index` = competition rank) — never `ns.rank = ns.rank + 1` (dense rank).
...
- **Stats analytics layer:** `games/worldcup/services/stats.py` exposes the public functions consumed by the public `/worldcup/stats` route. Public analytics routes must NOT use `@login_required`; build `my_picks` via `WorldCupPick.query.join(WorldCupTeam)` (never `enrollment.picks` — N+1). `get_ideal_lineup(country_stats)` powers the **"The Ideal Lineup"** Board-tab card — the highest-scoring buildable roster (top earner per tier slot: **9 teams**, not a "Perfect XI"/11). Pure over `country_stats`; returns `None` at 0 pts so the card hides pre-results.

...
- `games/docket/` — The Docket (NFL+CFB weekly pick'em) — **active focus**; registry `open`, NOT featured (Survivor keeps the lounge until the ~Oct multi-featured redesign). Light court-paper room; design doctrine in `games/docket/DESIGN.md`; binding rulings in the 2026-08-11 design SSoT (see the memory index). Pick sheet + join shipped (T7); sync/recalc CLI (T8), admin (T9), standings (T10), timers (T11) pending. The blueprint lives in `games/docket/blueprint.py`, not the package `__init__` — the pure grading package's flask-free import graph is a locked contract (D9). Prod docket tables stay empty until the deliberate Week-1 import at the real line freeze (Tue Sep 1).

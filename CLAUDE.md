...
- **WC hub state-shell primitives (PR #33):** Three small primitives extend the WC body doctrine.
  ...
  - `_home_live.html` **compressed dossier embed** (`.wc-dossier-sparkline` / `.wc-dossier-sparkline-empty` / `.wc-dossier-ledger` / `.wc-ledger-eyebrow` / `.wc-ledger-line` / `.wc-lk` + `.wc-lk--gold` / `.wc-lk--red` / `.wc-ledger-delta--up/down/flat`): parity port of the lounge `_dossier_card.html` into `.wc-standing-card.is-lead`. WC adaptation paints the sparkline path in `--wc-red` on bone (lounge uses gold on dark). `_context_live` returns the `dossier` dict (`sparkline_data`, `alive_count`, `week_delta_rank`, `week_delta_points`); the week-delta gate `len(recent_snapshots) >= 7` mirrors the lounge.
  - `_home_live.html` **`.wc-results-strip`** (sibling to `.wc-fixtures-strip`): demotes Recent Results from a full `.wc-stat-card` to an inline editorial strip. Reuses `.wc-fixtures-strip-head` + `.wc-fixture-side`/`-flag`/`-name`; adds `.wc-result-row` with `.wc-result-row.is-roster-match` for the points-earned roster highlight. Keep the lead → roster → strip → preview crescendo when editing live-state.
...
- **Home-page state shell:** `core/main/home_context.build_home_context(user, state)` dispatches to one of four per-state builders (`_context_out` / `_pre` / `_live` / `_post`); the home route resolves `state` from `worldcup_state()` and renders the matching `_home_<state>.html` partial inside `home_shell.html`. New home-page work goes through these builders — never recompute scoring or rank in the template. Live-state dossier sparkline + week-delta read from `WorldCupRankSnapshot` rows captured nightly by `flask worldcup snapshot-ranks` (idempotent; `--backfill N` seeds historical days). Week-delta is gated behind `len(snapshots) >= 7` to avoid overstating early-deploy trends. **The WC hub live state mirrors the lounge dossier via a compressed parity embed (PR #33).** `games/worldcup/services/home_context._context_live` returns a `dossier` dict with the same shape (`sparkline_data` / `alive_count` / `week_delta_rank` / `week_delta_points`), and `_home_live.html` embeds the sparkline + Newsreader ledger inside the `.wc-standing-card.is-lead`. The lounge stays the canonical rich surface (per the lounge/room architecture above); the hub embed exists so a halftime-bookmark user who lands on `/worldcup/` instead of `/` gets parallel beats. Same week-delta gate on both surfaces — keep them in lockstep when modifying either.

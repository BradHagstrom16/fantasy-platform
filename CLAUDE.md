...
FLASK_APP=app.py venv/bin/flask club desk --dry-run [--now ISO] [--anchor cfb,docket] [--ride F,S]   # One firing of the reminder desk: prints per member what would go (anchor letter, rider) and what would latch. SENDS NOTHING, WRITES NOTHING. --now is the ONE clock the run reads (naive = CT); --anchor = games the desk may lead for, --ride = slots (F=cfb/warning, S=cfb/final, D=docket/2h) that may carry a rider. Without --dry-run it sends and latches. LIVE since step 7: `cfb-remind.service`'s ExecStart is `flask club desk --scheduled --anchor cfb --ride F` (the desk leads Survivor's tiers; the Docket rides Friday's warning and docket-remind, `After=cfb-remind.service`, skips the pre-marked 48h tier); `cfb sync --mode remind` is the legacy pass kept only as the one-line rollback until step 9 (tests/test_cfb_timers.py locks the exact flags; tests/test_club_desk_matrix.py is the spec)

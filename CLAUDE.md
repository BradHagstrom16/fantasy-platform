...
- **Navbar wordmark + auth bust:** the navbar brand pairs the head mark with `wordmark-bone.svg` (`<img class="brand-wordmark d-none d-md-inline">` — head-only below md; the `<a class="navbar-brand">` carries `aria-label="Corrupt Commish Club"` for an accessible name when the wordmark is hidden). The auth desktop brand panel leads with `mascot-bust.svg` via the shared `core/auth/templates/auth/_brand_logo.html` partial. Wordmark kit: `static/img/logo/wordmark-{bone,gold,purple}.svg`; unwired brand-kit assets stay in the gitignored `CCC-final/`. Locked by `tests/test_logo_assets.py` + `tests/test_asset_versioning.py`.

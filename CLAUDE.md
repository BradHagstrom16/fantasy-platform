...
- **Display names (ADR-057): one per member, platform-wide.** `User.display_name` is the only member name; `CfbEnrollment`/`DocketEnrollment.get_display_name()` delegate to `user.get_display_name()` (no per-pool column — dropped by migration `b7c3e1a9d0f4`; the frozen WC enrollment keeps its column, which wins where set, then the same delegate). Every write site — `/register`, `/profile`, `POST /admin/users/<id>/display-name` — runs `utils/display_name.normalize_display_name(raw, exclude_user_id=…)`: strip + collapse whitespace, ≤ the column's 100 chars, blank ⇒ `None` (go by username), and a **soft** case-folded uniqueness check against every other member's display name AND username (no DB index, by the same decision as usernames). `username` stays immutable and never appears on a standings surface; join pages state `current_user.get_display_name()` and collect nothing. The 2025 CFB archive JSON is the one frozen-name store (by design). `tests/test_display_name.py`.
...
- **Scheduled jobs (droplet STATE 2026-08-22 — a snapshot; the truth is `systemctl list-unit-files 'worldcup-*.timer' 'cfb-*.timer' 'docket-*.timer' 'golf-*.timer' --no-pager`, no sudo):** every `deploy/*.timer` is installed by every deploy (ADR-041) and stays `disabled` until enabled by name. **Enabled:** all five `docket-*` (`setup` included — its hold-back reason, the first write to *empty* tables, lapsed with the 2026-08-19 preview import; against an existing week it only gap-fills, so Sep 1 still rides on the hand-run wipe-then-import) and `cfb-remind` (PR #169; no-ops hourly until the first real window, Fri Sep 4). **Launch-week enablement (2026-08-31 → 09-01, transition plan §6F):** `cfb-{spreads,scores,autopick}` on Mon Aug 31; `cfb-setup` on Tue Sep 1 *after* its Mon 06:00 CT slot has passed, so its first firing is Mon Sep 7 (creates Week 2 on cadence; scoring is activation-independent — `run_scores` iterates every incomplete week). Safe because **a freshly enabled `Persistent=true` timer does not catch-up fire on this droplet** (verified 2026-08-31: `docket-setup` enabled Wed 08-19 first ran Tue 08-25; `docket-deadline` enabled Wed 08-13 first ran Sat 08-15). Labor Day one-off: `cfb-scores` next fires Sun Sep 13 after Mon Sep 7 08:00, so hand-start `cfb-scores.service` Tue Sep 8 to score Week 1's Monday game. **Off for good:** `worldcup-*`. **Held until Phase L (~Jan 2027):** all six `golf-*` (the legacy PythonAnywhere app retired 2026-08-24; the units carry the free-tier cadence, ~115 API calls/mo — never widen one without re-doing that arithmetic). Systemd timers are canonical; the legacy crontab lines stay commented as history.

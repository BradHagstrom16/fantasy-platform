...
- `games/cfb/` — CFB Survivor Pool — **active focus**; registry `coming_soon`, flips to `open` at launch (~mid-Aug 2026; season starts Thu Sep 3). Dark-first room + admin cluster shipped; design doctrine in `games/cfb/DESIGN.md`.
- `games/worldcup/` — World Cup Fantasy Pool — **archived** (2026 tournament concluded 2026-07-19; permanent post-state). Registry stays `'open'` until the CFB changeover flips it to `'completed'`.
...
**Production:** Live at `cccfantasy.com`. CCC design system shipped at tag `impeccable-v1`. Product/design spine: `PRODUCT.md` + `DESIGN.md` (repo root); per-game specialization in `games/<slug>/DESIGN.md`. Any UI work invokes the `impeccable` skill, but its stock loader reads only the top-level docs — **hard rule: when working any UI surface under `games/<slug>/`, read `games/<slug>/DESIGN.md` alongside the top-level `DESIGN.md` before producing design output** (top-level owns cross-game/platform-foundation concerns; the per-game file owns that game's palette/accent-rank/register/named primitives; see `docs/per-game-design-doc-convention.md`). Update impeccable **only via `/update-plugins`** — never `npx impeccable skills update` from a repo root (drops a stray project-local copy; a guardrail hook blocks it).

**Architecture: lounge vs rooms.** Platform `/` is the **club lounge** — dark CCC purple+gold atmosphere, dominated by whichever single game is currently live. Each game has its own **room** with specialized identity (WC: light Casual-Light body; CFB: dark-first midnight room). **Substrate distinction between the lounge and a game body is by-design architectural separation, not whiplash** — don't try to converge substrates (small handoff polish is fine). Lounge state resolves through the registry's featured-game seam (C2 slice 1, shipped): `games.registry.lounge_game()` returns the single featured-open entry with a `lounge_state` resolver, and `core/main/routes.py` dispatches through it — the atomic changeover flip re-points the lounge by itself. Below the dispatcher the lounge is still WC-hardwired: the four `core/main/templates/main/_home_*.html` shells + `core/main/home_context.py` builders are WC-specific until C2 slice 2 splits them per game. The remaining generalization (per-game lounge modules, merged early gated on the changeover) is designed in the transition plan §5 + the C1 lounge spec — follow them rather than improvising a parallel approach.
...
- **Two parallel `_home_<state>.html` trees exist** — the game room's under `games/<slug>/templates/<slug>/`, the lounge's under `core/main/templates/main/` — with *distinct* primitives (e.g., the WC room's post banner is `.wc-champion-banner`; the lounge's is `.champion-banner` via `_champion_banner.html`). Don't cross-apply them. WC-room substrate doctrine (Casual-Light body, the champion banner as its only dark surface, scoped foreground carve-outs) lives in `games/worldcup/DESIGN.md`; the WC surfaces themselves are frozen (see World Cup section below).
...
- **Lounge state shell (pre-extraction):** `core/main/home_context.build_home_context(user, state)` dispatches four WC builders off `worldcup_state()`; never recompute scoring/rank in the lounge templates. This is exactly the code the transition plan §5 moves behind the featured-game seam.

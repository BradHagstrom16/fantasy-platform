...
FLASK_APP=app.py venv/bin/flask cfb sync --mode remind      # Pick reminders: hourly timer; windows T-26h35m..24h25m (day-before) and T-2h35m..25m (final) each span two firings, the second an outage retry; de-duped on CfbWeek.last_reminder_type; the final's copy says the time actually left (tests/test_cfb_timers.py, tests/test_cfb_reminders.py)
...
FLASK_APP=app.py venv/bin/flask docket sync --mode setup     # Create week + import slates + lock first-posted lines + rule-derived tiebreaker (Tue). Against an existing week, gap-fills only — locked lines are NEVER overwritten (runbook in games/docket/cli.py). Since ADR-065 step 5 the Tuesday run is club-paper.timer's (same setup, announced inside the Paper); docket-setup.timer is disabled at the cutover and retired in step 9.
...
FLASK_APP=app.py venv/bin/flask club desk --dry-run [--now ISO] [--anchor cfb,docket] [--ride F,S]   # One firing of the reminder desk: prints per member what would go (anchor letter, rider) and what would latch. SENDS NOTHING, WRITES NOTHING. --now is the ONE clock the run reads (naive = CT); --anchor = games the desk may lead for, --ride = slots (F=cfb/warning, S=cfb/final, D=docket/2h) that may carry a rider. Without --dry-run it sends and latches. LIVE since step 7, widened at step 8: `cfb-remind.service`'s ExecStart is `flask club desk --scheduled --anchor cfb --ride F,S` (the desk leads Survivor's tiers; the Docket rides Friday's warning and Saturday's final, and docket-remind, `After=cfb-remind.service`, skips the pre-marked 48h/24h tiers; its Sunday 2h stays its own); `cfb sync --mode remind` is the legacy pass kept only as the one-line rollback until step 9 (tests/test_cfb_timers.py locks the exact flags; tests/test_club_desk_matrix.py is the spec)
...
- **Scheduled jobs:** every `deploy/*.timer` is installed by every deploy (ADR-041) and stays `disabled` until enabled by name. Query the truth: `systemctl list-unit-files 'worldcup-*.timer' 'cfb-*.timer' 'docket-*.timer' 'golf-*.timer' 'scores-*.timer' 'club-*.timer' --no-pager` (no sudo). **Off for good:** `worldcup-*`. **Held until Phase L (~Jan 2027):** all six `golf-*` (free-tier cadence, ~115 API calls/mo — never widen without re-doing that arithmetic). **`club-paper.timer` opens BOTH games on Tuesday** (ADR-065 step 5; replaces `docket-setup.timer` and the Tuesday half of `cfb-spreads.timer`) and must stay enabled until Survivor's last CFP week in late January, which is after the Docket's season ends on Jan 12 — never disable it when the Docket goes quiet.

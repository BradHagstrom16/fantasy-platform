...
- **D16-eng — Bridge identity mapping:** the Form/mapping sheet carries a platform-identity column from Week 1; registration deadline **Sun Sep 13** [transcribed as "Sat Sep 13"; 2026-09-13 is a **Sunday**, the same weekday slip as the Sep 1 go/no-go — Saturday that week is Sep 12] in bridge comms; importer resolves via the mapping with a loud unresolved-identity report; all bridge players admin-enrolled via `/admin/enrollments` before import. *(Applies only if the parachute deploys.)*
...
- **D19-eng — Kickoff refresh:** every docket sync run also refreshes commence times from the **free `/events` endpoint** into the live kickoff column (lock enforcement + display); `kickoff_at_deadline` stays frozen; a moved game logs a visible change row. Closes the stale-kickoff/pick-a-live-game hole.
  - *As shipped (T8), narrower than "every docket sync run":* only `--mode setup` and `--mode lines` reach `import_week`, which is what performs the `/events` refresh (`games/docket/cli.py:337-344`). `scores`, `deadline` and `remind` do not. In practice `lines` runs Tue–Fri, so kickoffs are refreshed right up to the deadline that freezes `kickoff_at_deadline` — but the ruling says "every run" and the implementation does not, so the gap is recorded here rather than papered over.

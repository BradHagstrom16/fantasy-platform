...
- **[S3.1.1 cross-cluster] [S3.4 routed]** Orphan `.navbar-brand { color: var(--platform-accent) !important; }` rule at `static/css/style.css:~4019` paints the brand wordmark gold via lower-specificity `!important`, overriding `.navbar.navbar-dark .navbar-brand`'s spec-correct `var(--bone)`. DESIGN.md §5 Navigation specs "Brand (`.navbar-brand`): Teko 1.25rem, letter-spacing 0.04em, **Pressroom Bone** color." Rendered chrome is gold, not bone. S3.1.1's surgical scope was the gradient/!important cascade fix + 44×44 floor + sub-nav contrast + skip-link; the brand-color call is a meaningful Trophy-Rule-adjacent decision (does the wordmark compete with the gold "Join the Club" trophy CTA?) that wants cross-chrome comparison against the auth pages (S3.2) before flipping. Routed receiving session: **S3.4** (chrome cross-cluster polish). Resolution choice belongs to S3.4: either (a) drop the orphan `!important` and restore DESIGN.md §5 spec (preferred), or (b) amend DESIGN.md §5 to specify gold-wordmark + add a regression-lock.
- **[S3.1.1 cross-cluster] [S3.4 routed]** No explicit `:focus-visible` styling on `.navbar .nav-link` or `.subnav-pill` — keyboard focus inherits the browser-default outline ring, which is inconsistent across browsers. Skip-link uses `outline: none` paired with the off-screen-to-onscreen reveal (focus IS the appearance change), which is acceptable for that primitive. The nav-link / subnav-pill focus visibility is a chrome a11y gap that benefits from cross-cluster comparison against auth-page form-control focus rings (S3.2 already locks gold-glow focus on form controls). Receiving session: **S3.4** (chrome cross-cluster polish).
- **[S3.1.1 cross-cluster] [S3.4 routed]** `.game-subnav` `aria-label` missing on the container — on mobile the `.subnav-game-label` text is `display: none` via `d-none d-sm-inline-flex`, so screen-reader users on mobile lose the game-context cue ("Hub of what?"). An `aria-label="World Cup section navigation"` on the wrapper closes the gap without affecting sighted users. Receiving session: **S3.4** (also touches global chrome semantics + per-game labels — cluster pattern).
...
- **[S3.2.1 cross-cluster] [S3.4 routed]** Split-panel (`auth-page > .auth-panel-brand + .auth-form-panel`) vs `.auth-wrapper` layout split between marketing-context auth (login, register, forgot, reset — 4 surfaces) and logged-in utility auth (change-password, profile — 2 surfaces) is currently undocumented. Both share the `body.auth-page` Tribunal Black backdrop (correct per DESIGN.md §4.4), but the in-card composition contract isn't ratified. Receiving session: **S3.4** (chrome cross-cluster polish) — either codify the two-pattern split in DESIGN.md §5 with a regression-lock, or unify to a single pattern.
...
- **[S3.3.1 cross-cluster] → S4.1** `_home_out.html:75-88` 3-up `col-md-4` identical-card-grid composition (1 logged-out + 2 coming-soon cards at the same size and silhouette). The S3.3.1 PI-A within-partial differentiation (drop `.card-footer` on `coming_soon`, eyebrow-position badge above icon, softer opacity .78 + grayscale 18%) gave the two states distinct silhouettes inside `_game_card.html`; the composition-level reshape (1 featured WC card + paired coming-soon strips, or sized-down "next games" rail) belongs to S4.1 when it reshapes `_home_out.html`. Anti-pattern detector still fires on the surface as long as the 3-up grid persists. Receiving session: **S4.1** (in-surface for `_home_out.html`).
- **[S3.3.1 cross-cluster] → S4.1** Missing `<h2>` section heading between the `_home_out.html` page `<h1>` ("The Fix" / "Is In.") and the registry-driven game-card grid (`:75-88`). Axe heading-order moderate violation: page jumps `<h1>` → `<h3>` (cards now at h3 per S3.3.1 F3 partial fix). Adding an `<h2>` like `<h2>Take a seat at the table.</h2>` (or whatever S4.1 settles on for the section eyebrow) closes the gap and earns the cards a parent heading the screen-reader outline can hang off. Receiving session: **S4.1**.
- **[S3.3.1 cross-cluster] → S4.1** `_home_out.html:68` "Sign in" link inside the join-alt row (`Already sworn in? <a href="...">Sign in</a>`) probes at 44×15 — fails the PRODUCT.md mobile 44×44 tap floor (height-only fail). Self-contained CSS or markup fix; lives in an `_home_out.html` block, not in a S3.3.1-scope partial. Receiving session: **S4.1**.
- **[S3.3.1 in-surface]** `_game_tiles_compact.html:28-29` carries slug-branched display copy in a presentation partial: `{{ game.display_name.split()[0] if game.slug == 'cfb' else 'Golf' }}` and `{% if game.slug == 'cfb' %}Sep 3{% else %}2027{% endif %}`. The `display_name.split()[0]` is brittle (any registry rename like "College Football Survivor" → first word `College` breaks the label) and the hardcoded `Sep 3` / `2027` launch dates live invisible to anyone editing `games/registry.py`. DESIGN.md §1 "Each game provides its own room: its own palette, sub-nav, and interaction emphasis. Game blueprints inject their identity"; PRODUCT.md Design Principle 5 "Custom Games Earn Custom Layers" — per-game identity should flow from the registry/blueprint, not from ternaries in a shared partial. Two clean shapes: (a) add optional `short_name` (default `display_name`) and `launch_label` (default `'TBA'`) fields to `GameRegistryEntry`, partial reads `{{ game.short_name }}` / `{{ game.launch_label }}` with no slug knowledge; (b) hoist into `core/main/home_context.py` as pre-resolved `tile_meta` dicts and pass them into the template. Receiving session: **S3.3.2**.
- **[S3.3.1 in-surface]** Lift S3.3 heuristics from 24/40 to ≥26/40 (gate (b) baseline+6 floor for first iteration). Two small polish PIs sized to close the gap: (a) `$impeccable clarify` inline "Opens [season]" microcopy on `_game_card.html` `coming_soon` variant — closes Heuristic 10 (Help and Documentation) from 2 → 3 by giving each coming-soon card a temporal anchor without requiring a separate countdown; copy can come from the same `launch_label` registry field that closes the slug-branch backlog item above (kill two birds with one PI if S3.3.2 lands them together); (b) `$impeccable polish` `.game-card--live :focus-visible` outline crispness to match `.btn-warning`'s gold focus ring — closes Heuristic 7 (Flexibility and Efficiency of Use) from 2 → 3 by giving keyboard-first users an obvious arrival indicator on the home grid's playable cards (currently inherits browser default, inconsistent across browsers per the S3.1.1 routed finding on `.nav-link` / `.subnav-pill`). Receiving session: **S3.3.2**.

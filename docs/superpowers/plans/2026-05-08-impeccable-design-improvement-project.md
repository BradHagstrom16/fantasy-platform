...
- **[S5.1.1 in-surface] → S5.1.2** Podium + Final Roster `<a>` text-link tap targets render 15px tall at 375 mobile (`B1G_Brad` 68×15, `test2` 33×15 — well below the 44 floor). Pattern is universal across both `_home_post.html` tables (Final Podium player anchors + Final Roster team anchors). Self-contained CSS fix (`.card.wc-card .table a { display: inline-block; min-height: 44px; line-height: 44px; }` or row-wrap pattern); routed to next iteration so S5.1.1 stays focused on the Tribune-voice lift (hero-metric collapse + retrospection line + eyebrow disambiguation) called out by plan §7. Receiving session: **S5.1.2**.
- **[S5.1.1 cross-cluster] → S5.3** `.card.wc-card .wc-numeral` (style.css:2848) renders bone (`#F3EFE6`) on the `.row-champion-pick` cream substrate (1.05:1, axe-confirmed) and on the masked-white substrate of every `.card.wc-card .table` cell (1.14:1 — axe surfaces 8 hits on _home_post tables alone) since S2.6 PI-1's `--bs-table-bg: var(--bg-card)` lock made all cells render on `--bg-card` (white). The bone override was authored when `.wc-numeral` spans actually rendered on the navy parent; post-S2.6 the in-cell numerals sit on white and read as near-invisible bone tint. Cross-cluster scope (verified by selector breadth): leaderboard, player_detail, picks, schedule, team_detail all use `.card.wc-card .table .wc-numeral`. The fix shape is selector-scope: retire the bone color on `.card.wc-card .table .wc-numeral` (Bootstrap default ink inherits from `--bs-table-color`, ~16:1 on white) while leaving the override intact on direct-navy `.wc-numeral` spans (the masthead `.post-finish-rank` already declares its own bone color independently). Receiving session: **S5.3** post-cluster polish (touches multiple post-state surfaces; verify the cross-cluster premise holds outside post then re-route to S6.1 if it spans every WC table).
- **[S5.1.1 cross-cluster] → S5.3** `.btn-outline-secondary` quicklink trio in the home_shell footer (`Schedule` / `Groups` / `Rules`) reads `#8a849b` on `#313d53` at 3.04:1 (axe-confirmed). The buttons are universal across all four state partials (rendered by `home_shell.html:48-62`, not in `_home_post.html` itself), so the contrast hit lands every state. Bootstrap's `.btn-outline-secondary` token is the source — re-tinting to `--bone-mute` on `--game-primary` would land cluster-wide. Receiving session: **S5.3** (touches the home shell, scope spans every state partial — verify pre/live/out parity at S5.3 step 2).

...
- **[S2.2.1 cross-cluster]** No jump-to-today affordance from above-the-fold on `/worldcup/schedule`. The `id="today"` anchor exists; nothing in the hero links to it. Mid-tournament the today block sits 6-11 matchdays into the page, so live-state surfacing requires a scroll. Pattern likely shared with future live-cluster surfaces (team_detail "Recent" / stats "Today" anchors). Receiving session: **S2.6** (live cluster polish).
- **[S2.2.1 cross-cluster]** Stage-count `<small>` in the schedule section headings uses Bootstrap `.text-muted` (`#6c757d`) instead of the CCC tinted `--text-secondary` (`#5A5470`). Marginal AA pass either way, but a tonal-consistency leak vs. the day-header on the same surface. Likely shared with similar `<small>` count helpers across live-cluster surfaces. Receiving session: **S2.6**.
...
- **[S2.3.1 cross-cluster]** Eyebrow primitive saturation. `.wc-eyebrow` renders 9-14× per page on team_detail (hero pre-headline, hero stat labels, ribbon labels, fixture stage rows, path stage tiles, picker section). DESIGN.md §3 defines the primitive as "the small uppercase line above section headlines", singular. The new `.wc-meta-label` primitive (introduced in S2.3.1 for the hero stat caption) is a candidate for in-row labels but needs cluster-level review against home/leaderboard/stats/player_detail before promoting it. Receiving session: **S2.6** (live cluster polish).
...
- **[S2.4.1 cross-cluster]** Inline-style Teko declarations duplicated ~25× across `stats.html` JS render functions (`font-family:'Teko',sans-serif;font-size:.7rem;...`). Pattern likely shared with other JS-rendered surfaces (home _home_live impact rows, leaderboard mobile cards). Extract to a `.wc-microcaption` utility set after auditing cross-surface usage. Receiving session: **S2.6** (live cluster polish).
- **[S2.4.1 cross-cluster]** `.wc-stat-card` carries both `box-shadow: var(--shadow-sm)` AND `border: 1px solid var(--border)` — double elevation. Pattern likely shared with `.team-detail-card` / leaderboard mobile cards / other `.wc-*` body cards. Either the shadow or the border earns its place, not both; pick one and apply consistently across the live cluster. Receiving session: **S2.6**.
...
- **[S2.5.1 cross-cluster]** Bootstrap-on-`.card.wc-card` contrast leak is a cluster-wide latent risk. S2.5.1's PI-1 fix is scoped to `.player-picks-desktop`. picks.html (the user's *own* picks editor at `games/worldcup/templates/worldcup/picks.html`) uses the same `.player-picks-desktop` class on the same dark surface and inherits the fix automatically. But other `.card.wc-card`-wrapped Bootstrap tables across the cluster — leaderboard fragments, admin lists, future state-cluster tables — won't, because the cluster-3 surgical-exclusion at `style.css:5444` *expects* a Bootstrap white-td bg masking the navy. Any future `.card.wc-card` surface that does NOT have white-td masking will re-surface the same dark-on-dark bug. Receiving session: **S2.6** (live cluster polish should sweep for any other `.card.wc-card .table` pattern and either lock the white-td assumption or extend the counter-rule).

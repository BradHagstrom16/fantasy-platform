...
- **[S4.4.1 cross-cluster] → S4.5** Pill-rail `.wc-group-index-pill { min-width: 44px }` × 12 letters + gaps yields a 540px+ wide rail at 1470px viewport. The 44×44 floor is non-negotiable on mobile (PRODUCT.md tap-target hard requirement); the question for the cluster polish session is whether the rail should cap-out at a content-narrow `max-width` on `≥xl` to stop stretching on ultra-wide canvases. Compare against the schedule.html `.schedule-jump-today` chip (S2.6 PI-4) and the rules.html TOC-routed item already on the S4.5 docket — the consistent in-page anchor pattern decision lives at cluster altitude. Receiving session: **S4.5**.
...
- **[S4.3.1 cross-cluster] → S4.5** Mobile tier-meta text below 16px body floor. `.tier-mobile-card-picks` (13.6px) and `.tier-teams-list` (13.6px) on `rules.html` mobile tier cards sit below the 16px mobile body-text floor. Defensible as secondary-metadata-on-progressive-disclosure surface (the tier card's primary signal is the dot + Teko name + Multiplier chip; the picks-count + teams-list are caption-tier), but the cluster polish session should compare against the picks-card metadata sizes (mobile `.player-pick-card` lines) + team-detail fixture-row meta + groups-page-to-come cell typography to pick either a consistent caption-tier floor or grant explicit metadata-tier dispensation in DESIGN.md §3. Receiving session: **S4.5**.
- **[S4.3.1 cross-cluster] → S4.5** Rules-page navigation accelerators. Long-form (~3300px desktop, ~5000px mobile) reference with seven H2 sections has no in-page TOC, no jump-to-section anchors, and no skip-link to bypass the desktop tier table for screen-reader users. Analyst-register power-user gap. Cluster polish session should compare against the schedule.html `.schedule-jump-today` chip precedent (S2.6 PI-4) and decide a consistent in-page anchor pattern across long surfaces (rules, schedule, picks accordion). Receiving session: **S4.5**.
...
- [x] **S4.4** — groups (converged: 2026-05-11, S4.4.1)
  - [x] S4.4.1 (commit: ____) — heur 24→30/40 (Δ +6), audit 13→18/20 (Δ +5), anti-pat hard hits 3→0. Gate: **PASS** path (b) baseline+6 floor (30 exactly).

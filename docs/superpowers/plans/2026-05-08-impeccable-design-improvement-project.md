...
- **[S4.2.1 in-surface] → S4.2.2** Three competing tier-vocabulary primitives on one page: `.tier-badge` (pill, light surface, used in sidebar pick-summary + mobile readonly card), `.wc-multiplier-chip` (dark-surface chip, used in desktop readonly table + tier-card-header), `.wc-tier-dot` (compact circular dot). Same data, three different shapes. Heuristic #4 (Consistency) capped at 2 because a casual user has to relearn the tier visual on each shell. Fix: pick `.wc-tier-dot` + Teko name as the primary tier primitive (smallest cognitive load, works on light AND dark); demote `.tier-badge` to the sidebar pick-summary only (one place, one job); restrict `.wc-multiplier-chip` to multiplier values only (never tier-encoding doublespeak). Touches `picks.html:90` (mobile readonly), `_pick_row.html:19-23`, `picks.html:311` (summaryList JS builder). Receiving session: **S4.2.2** (recommended `$impeccable distill`).
- **[S4.2.1 in-surface] → S4.2.2** Inline `style="font-size:N.Nrem"` type-scale leakage on `.wc-numeral` spans (`picks.html:42, 78, 82, 90, 109, 198` = 6 instances; re-critique counted 12 total including mobile readonly `.tier-badge` font-size). Bypasses the DESIGN.md fixed-scale system. Extract `.wc-numeral--xl` / `.wc-numeral--lg` / `.wc-numeral--sm` modifier classes (or fold into the existing display/title/headline scale). The mobile readonly `<h2 class="pick-card-head--mobile">` carrying `style="font-family:'Teko',...,font-size:1.1rem"` was already partially closed in S4.2.1 F2 (the inline style was dropped; the new `.pick-card-head--mobile` class carries the 1.1rem). Remaining: 12 numeral / badge font-size inline declarations. Receiving session: **S4.2.2** (recommended `$impeccable typeset`).
- **[S4.2.1 in-surface] → S4.2.2** Mobile `.player-pick-card` is non-interactive — no tap-through to `/worldcup/team_detail/<id>` (desktop readonly row has `.team-link` to team_detail). Casey gets a worse drill-down than the desktop user on the same data. Linkify the card OR add the accordion drilldown (the same `_pick_accordion_script.html` engine could be reused). Wrap option (cheaper): `<a href="{{ url_for('worldcup.team_detail', team_id=pick.team_id) }}" class="player-pick-card-link">`. Receiving session: **S4.2.2** (recommended `$impeccable adapt`).
- **[S4.2.1 in-surface] → S4.2.2** `.pick-summary` rendered via `border-top: 3px solid var(--game-primary)` (literal WC navy resolves via `--game-primary`). Side-stripe-adjacent pattern (the impeccable absolute ban targets `border-left/right` >1px; a 3px top-stripe lives in the same family). Either lift to full-border game-tinted edge or drop the stripe and let the Teko h4 + eyebrow carry hierarchy alone. Re-critique re-measured the live computed color as `solid rgb(0, 40, 104)` — raw WC navy, not even tokenized. Receiving session: **S4.2.2** (fold into `$impeccable colorize` or `$impeccable polish`).
- **[S4.2.1 in-surface] → S4.2.2** `.wc-team-card.selected::after` uses `content: '\2713'` (Unicode checkmark glyph). Renders correctly but the markup-as-icon-glyph pattern composes poorly with the type scale and is adjacent to the soft DESIGN.md "don't render meaning via unstyled glyphs" register. Swap to an inline SVG or a Bootstrap Icons `bi-check2` pseudo-element. Single CSS rule edit. Receiving session: **S4.2.2** (fold into the `$impeccable polish` sweep).
- **[S4.2.1 in-surface] → S4.2.2** Edit-form heading-order H1 → H3 skip. The S4.2.1 F2 fix repaired the readonly card-header (H1 → H4 → H2); the edit form has a parallel issue — the page H1 ("Submit Your Picks" / "Amend the Oath") followed by `<h3 class="d-flex align-items-center gap-2 mb-0">` inside `.tier-card-header` ×5 with no H2 between. Sidebar `<h4>` "Pick Summary" (line ~194) is also still an H4. Promote sidebar h4 → h2; insert `<h2 class="visually-hidden">Build your roster</h2>` before the tier-cards loop OR a visible "The Ballot" / "Build your roster" Teko h2 above the tier-card list. Receiving session: **S4.2.2** (`$impeccable adapt` or single-PI `$impeccable harden` continuation).
- **[S4.2.1 in-surface] → S4.2.2** `.wc-multiplier-chip` references `var(--wc-white)` — DESIGN.md `colors:` block does not define `--wc-white` (the canonical token is `--bone` / `--text-on-dark`). Chip currently renders `rgb(245, 241, 232)` via tokens.css fallback chain, so visually intact. Token-name hygiene: swap to `var(--text-on-dark)` to match the same fix pattern applied to `.tier-badge` in S4.2.1 PI-4. Single-line CSS edit, no visual delta. Surfaced by S4.2.1 re-critique. Receiving session: **S4.2.2** (fold into the `$impeccable distill` tier-vocabulary collapse).
- **[S4.2.1 cross-cluster] → S4.5** "Base · Multiplier · Points" derivation-table column trio in the desktop readonly picks table reads as a spreadsheet (PRODUCT.md anti-reference "Spreadsheet-shaped information hierarchy"). The same shape likely lives on `team_detail.html` (per-match fixture column) and `player_detail.html` (pick rows). Cluster-altitude decision: keep the 3-column derivation visible, or collapse to 2 columns + accordion-only base reveal. Verify cross-surface presence at session time per S2.6 calibration. Receiving session: **S4.5** (cross-cluster polish).
- **[S4.2.1 cross-cluster] → S4.5** `.tier-badge` vs `.wc-multiplier-chip` vocabulary overlap is wider than picks alone — both primitives are used on `team_detail.html`, `player_detail.html`, and `stats.html`. The S4.2.2 in-surface fix collapses to picks; the cluster-altitude ratification (which primitive belongs in DESIGN.md §6 as canonical) happens at S4.5 after picks / rules / join converge. Receiving session: **S4.5**.
...
- [ ] **S4.1 — `_home_pre.html` + `_home_out.html`** (the *platform* home in pre + logged-out states, dispatched from `core/main/routes.py` via `core/main/templates/main/index.html`). Original plan label said "WC home in pre states" — actual surfaces resolved to the platform home partials per S3.3.1's routed-in items and the dossier composition (countdown decree + ballot card + join CTA + opening matches). The WC game home (`games/worldcup/templates/worldcup/_home_pre.html` + `_home_out.html`) is a separate surface served at `/worldcup/` and is NOT part of S4.1 scope. Likely Priority Issues at first iteration: countdown card emotional fatigue if user visits often, ballot card readability, Tribute Window framing.
  - [x] **S4.1.1** — first iteration; landed PI-1 (`$impeccable layout` — registry grid reshape closing the S3.3.1-routed identical-card-grid + missing-`<h2>` items as one atomic edit) + PI-2 (`$impeccable distill` — countdown 4-cell hero-metric adjacency collapse to one dominant Days numeral + Newsreader derivation) + PI-3 (`$impeccable adapt` — `.decree-links` 44×44 tap-target floor) + F1 (S3.3.1-inherited `.join-alt a` 44×44 floor). Gate: **FAIL** on §1.5b condition (4). Iteration continues at S4.1.2.

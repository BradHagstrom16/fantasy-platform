...
- [ ] Enable the five CFB timer pairs: `sudo systemctl enable --now cfb-*.timer`, then confirm with `systemctl list-timers 'cfb-*'`. **No copying step** — since ADR-041 (PR #123) `deploy.sh` installs every unit in `deploy/` on every deploy, so the `cfb-*` units are already present in `/etc/systemd/system/`, `disabled`. Verify that before enabling (`systemctl list-unit-files 'cfb-*'`); if any are missing, the deploy did not run, and hand-copying is the wrong fix. Do **not** un-comment the legacy CFB crontab lines — timers are the canonical mechanism; delete or leave the commented cron lines as history.

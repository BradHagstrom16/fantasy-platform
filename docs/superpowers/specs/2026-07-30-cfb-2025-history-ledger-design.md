...
4. **Final standings** — 26-row ledger: player, outcome ("Champion" / "Out · Week N"), final lives as shipped `.lives-indicator` pips (with text labels for color-independence), cumulative spread. Ordered champion first, then out-week **descending** (longest survivors high); within a shared out-week, lower cumulative spread first (row-order determinism mirroring the platform tiebreak convention, not a claimed official rank). **No invented rank column** — the 2025 pool never published ordinal ranks for eliminated players; order + out-week tell the story without minting false precision (doctrine §10.3).

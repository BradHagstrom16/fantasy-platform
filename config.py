...
class ProductionConfig(Config):
    ...
    # flask_wtf.csrf.CSRFProtect.protect() runs AFTER the signed-token check
    # and additionally rejects the request unless the browser's Referer origin
    # byte-matches https://<request.host>/ (same_origin: scheme+host+port).
    # Behind Cloudflare that comparison is unreliable — request.host is
    # proxy-driven (ProxyFix x_host=1) and www-vs-apex, translated pages
    # (*.translate.goog), and privacy/proxy referrers all differ from it,
    # producing "Bad Request — The referrer does not match the host." on login
    # and password reset (reported by a real user 2026-08-21). This flag
    # disables ONLY that referrer sub-check; the cryptographic double-submit
    # token check (the actual CSRF protection) stays fully active.

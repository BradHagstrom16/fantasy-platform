...
- **Cumulative-spread tiebreaker is lifetime** (no CFP reset): the sum of each pick's signed spread from the picked team's side (a favorite's minus counts against you, an underdog's plus counts for you); **higher is better** because the harder road ranks above the safe one; a week's picks enter the total only once that week's deadline passes (a Thursday kickoff locks the pick, it does not settle the spread, and the standings must not leak a hidden pick through its spread); No Contest excluded. Ruled 2026-09-01 after Week 1 shipped the legacy ASC order.
...
- **OPEN** — the week is active (since ADR-062 that means its lines have landed: a week opens on the spreads run that posts its first line, never before), the deadline has not passed, and the player has **no valid pick**. Emphasize: available action, remaining time, eligible teams, and the consequence of not acting. This is the highest-urgency state.
...
| Champion moment | The champion's name (no H1 element; the name is the heading, with no eyebrow above it, ADR-066, matching the Docket's verdict banner) |
...
**Never above a heading (ADR-066, Brad 2026-09-24; closes the open question the Field raised).** No hero carries an eyebrow, on any tab: the H1 speaks, and a fact the eyebrow carried moves into the heading, the lead or the hero field line ("Your Week 4 Card"; "Week 4." opening the results lead; the archive year in the history field line; a week in the admin H1s, "Mark the Week 4 Results"). Nor does one sit over an in-page headline: the week lead, the season-standing cards (the viewer's "You" tag rides the headline), The Pool and the champion's name lost theirs ("One Remains" on the live champion hero and "One Remained" on the 2025 archive; Brad 2026-09-25, to match the Docket's verdict banner. The subtitle "CFB Survivor Pool Champion" and the archive's "1 remained" field line keep the fact). What stays is a label over something that is not a heading: "Picks Lock" over the countdown, "Your Standing Pick" over the held team, "Your Verdict" over the result, "The Spent Board" over its note, the Field's fold heads. The platform rule is root `DESIGN.md` The Eyebrow Rule; it is restated here because impeccable's `--target games/cfb` loads this file instead of the root one. Locked by `tests/test_eyebrow_above_heading.py`.
...
- **The Crimson-Ceremony Rule (no gold in CFB)** — CFB does not adopt gold as a game-level ceremonial organizer; that is WC's move, and a dark room reaching for gold would read as WC. CFB's ceremony is crimson restraint on midnight. Platform gold stays where the platform owns it as *chrome* (the navbar CTA, platform focus rings outside the room, and the lounge's shared frame — the seam, the seal band, the eyebrow glyphs); CFB never introduces gold into its body or ceremonial surfaces — including the champion state, whose strongest signals are space, singularity, finality, and evidence, not color. **Since ADR-052 (2026-08-19) this extends to the CFB lounge panel buttons: "Enter the Room" and "Choose Team" are solid crimson `.hl-cta`, not the former metal-gold `.cta-seal`, so CFB now uses no gold button anywhere — the lounge seal was its last carve-out.** (Shipped and test-locked: the hero dot overlay, admin masthead, and focus rings all went crimson; the lounge CTAs by `tests/test_lounge_cta_parity.py`.)
...
**The board and the bar (impeccable critique 2026-09-01, live Week 1 at 375px).** Three contracts, all test-locked (`tests/test_cfb_pick_board.py`):

- **The board is the answer set.** A game shows by default iff at least one side is pickable, or it holds the member's existing pick; every other game sits in the DOM behind `.cfb-board-rest`, a native `<details>` whose 44px summary reads "Show all N games" / "Show open games only". Hidden cards keep their reason chips verbatim (collapsed, never omitted), and the absence is stated above the toggle ("28 games have no open team this week: opponents outside the pool, favorites of 16.5 or more."). The disclosure opens by default when nothing is pickable, so the exhausted state still shows the whole slate. Board state is computed once in `games/cfb/services/board.py` (one precedence per side, one label dict shared by chips, legend, and pool ledger, a legend that is a **census** of what rendered rather than a phase list) and the template renders it; the card body is a macro so both halves of the board stay byte-identical.
- **Open rows have a positive treatment.** A resting open row carries an inset strong hairline; open vs out was a 1.10:1 background step and "open" read only as the absence of a chip. Inside an open card the non-pickable side is the opponent, not a peer: `.team-option-out.is-opponent` demotes it to a compact line under the 44px floor. The legend's Open swatch mirrors the resting row (raised + hairline), never a crimson the board does not show at rest. Chips (`.cfb-out-reason`) sit at the `.75rem` caption floor and speak member language: "Favored 16.5+", "Not a Pool Team", "Already Used", "Kicked Off", "No Game This Week".
- **The commit bar is sticky and names the team.** `#pickConfirmBar` pins to the viewport bottom on `--cfb-lifted` (the ramp's sticky-bar step) the moment a row is tapped, so the control follows the thumb (it sat ~6,900px below the first card, and a tap looked like a submission). Eyebrow "Your Call · Not Locked Yet" until the POST; "Your Standing Pick" while it shows the held team. The button is "Lock In {Team}" / "Change To {Team}" / "Keep {Team}"; "Clear" is a real deselect (back to the held pick, or an empty card), never an exit. Above the board a status strip states lives, the clock as relative + absolute ("Picks Lock in 3d 2h · Saturday, Sep 5 · 11:00 AM CT"), and the unresolved obligation with its consequence ("You have not made a pick. Miss the deadline and you get the biggest available favorite."), followed by the one rules line and the caption that says what a minus sign means.

**JS hooks:** the pick flow is driven by `data-team-id` / `data-game-card` / `.team-option`, plus `#confirmBtnTeam`, `#confirmEyebrow`, `#clearSelection`. Audit every `querySelector`/`data-*` hook before renaming any class; add CSS classes alongside JS-critical ones rather than renaming (platform template-restyling rule).
...
Official order, player, lives, active/eliminated state, the **reveal week's** pick state (the latest week whose deadline has passed, complete or not — never the active week before it locks: after Tuesday's spreads run opens the next week (ADR-062) a Monday-night or midweek game is still pending, and the field's picks stay on the board with their own LOCKED/VERDICT chips until the next week locks; one factual `.cfb-field-note` says so; ruled 2026-09-07, `tests/test_cfb_standings_reveal.py`), cumulative spread, tiebreak context. Current-user row easy to locate ("You" tag + tint — §7.2). Active players visually distinct from eliminated (separate section or controlled collapse; eliminated players remain inspectable — their history is part of the season story). Sorting only where it supports legitimate inspection, never implying an alternate sort is official; tied players don't get invented unique ranks unless the product convention supports them.
...
- **Spread conventions are league law:** locked at first fetch (manual entries also lock); signed display (`-7.5`, `+3`); cumulative = lifetime sum of the signed spread from the picked team's side (favorite negative, underdog positive), higher is better, counted only after the week deadline, No Contest excluded; historical cumulative spread never retroactively changes because an external line moved. Official ordering is implemented once, centrally — table sort logic must never become the de facto rules engine.

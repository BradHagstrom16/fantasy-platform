"""WC lounge context builders (transition plan §5, C2 slice 2).

Per-state data assembly for the platform lounge (Spec B section 4) while
the World Cup owns it — moved essentially as-is from
core/main/home_context.py behind the registry seam
(``GameRegistryEntry.lounge_context``). The core dispatcher
(``core.main.home_context.build_home_context``) supplies the
registry-generic keys (available/joined/coming-soon games) and the commish
note; everything here is WC data. The matching partial tree lives at
games/worldcup/templates/worldcup/lounge/.

Not to be confused with games/worldcup/services/home_context.py — that is
the WC *room's* home builder (the parallel tree per CLAUDE.md); this module
is the *lounge's*.
"""
...
def build_lounge_context(user: Any, state: WorldCupState | None) -> dict:
    """Assemble the WC contribution to the lounge context for the given state.

    state=None for unauthenticated users (logged-out marketing surface).
    For authenticated users, state must be 'pre' | 'live' | 'post' (the
    WC entry's lounge_state resolver, worldcup_state()).
    """
    if state is None:
        return _context_out()
...
def _context_out() -> dict:
    """Logged-out marketing surface — WC's contribution is the enrolled

...
**The second bill** (`.second-bill`, T13). The lounge is dominated by one featured game, but an *open* game that does not own the lounge would otherwise render nowhere — the compact tile strip only feeds off the featured entry, the archived tiles, and the coming-soon list. `.second-bill` is that game's presence: a registry-generic strip (mark, name, tagline, static cadence copy, enrollment-aware outline CTA) rendered from `games.registry.second_bill_games()`, sitting immediately above the compact tiles in whichever container holds them. It takes the Informational recipe, not Ceremonial: it is static by ruling (D21-eng) and asks nothing before a deadline. Its CTA is `.cta-outline`, never `.cta-seal` — the metal-gold budget belongs to the featured game. This is the interim shape; the ~Oct multi-featured redesign replaces it, and until then the single-overlay seam (`lounge_game()` + the `build_home_context` merge) stays untouched.

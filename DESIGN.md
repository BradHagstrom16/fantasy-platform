...
- **The lounge** — platform home (`/`). The club's own dark surface: purple radial atmosphere, gold ceremony, the Commissioner's voice. It bills every headliner as an equal — since the 2026-08-18 multi-featured rework, CFB Survivor and The Docket share it as co-headliners (The Undercard: two accent-washed panels meeting at a thin gold seam) — but its identity is always the club's: a game's room palette never restyles the lounge substrate. Games enter the lounge through **content, copy, state, and their lounge accent signature** (ADR-049): each headliner's panel and conversion card may wear its game's accent as thin strokes — the panel wash, a hairline border, tinted chips, the accent CTA fill — via the `--lounge-*` tokens in tokens.css and the `--hl-*` mapping only, never the room's `--game-*` variables, never room-scoped classes, and never accent-colored text below large/bold sizes (both accent families are dark pigments). Gold is the shared chrome only: the seam, the seal band, the hero eyebrow, the ◇/◈ glyphs. Since ADR-052 (2026-08-19) no lounge panel button is gold — every action wears the game's own solid accent `.hl-cta` (crimson / garnet) in every state; the metal-gold `.cta-seal` is retired from the headliner panels and survives only in the frozen WC archived lounge. Three 2026-08-18 design-review amendments (ADR-051): the decree seal band is platform lounge vocabulary worn by **every** headliner's pre panel, numbered per era (No 001 WC, No 002 CFB Survivor, No 003 The Docket — band only outside CFB; the asks keep their accent fills); the season ledger is a **shell-owned full-width Informational strip** (`main/_lounge_ledger.html`, fed by a registry-generic `ctx['ledger']` lift), never a panel interior; and the shell greet is enrollment-aware — a joined-nothing viewer gets the welcome/Choose Your Games variant while any join window is open, and the instruction retires with the asks (ADR-050).
...
- **Eyebrow** — the platform foundation defines one primitive; games may add their own variants:
    ...
    - **An eyebrow never sits above a heading** (ADR-066): see The Eyebrow Rule below.
...
**The Eyebrow Rule (ADR-066, 2026-09-24).** No eyebrow above a heading: the heading carries its own weight (the impeccable craft floor's one ban no brief earns back). An eyebrow is a label that heads something that is not a heading: a value ("Picks Lock / in 3d"), a stamp ("Filed"), a list or paragraph, a fold's summary, a desk letter's game section. When a heading needs context (game, week, state, rank), put the fact in the heading ("Your Week 4 Card", "The Docket, Week 3: 5-3") or in the line under it (the lead, the hero field line), never in a label over it. Where a label is right, reach for the matching Eyebrow primitive (`.admin-eyebrow`, or the game's variant in its `DESIGN.md`); don't invent a "kicker" or "subhead" pattern. Enforced in the CFB and Docket rooms, the Settle the Tab card and every member email (`tests/test_eyebrow_above_heading.py`). Out of scope: the lounge, whose ◇ summons eyebrows are sanctioned lounge chrome (ADR-052), and the frozen World Cup. Not yet applied: the platform admin masthead, the auth cards and `/app`; they meet the rule when next reworked, never by a new eyebrow over a heading.
...
**The second bill** (`.second-bill`, T13 — dormant since the multi-featured rework). Under the multi-featured lounge every open featured game is a headliner with its own panel, so `games.registry.second_bill_games()` returns an empty list today and the strip renders nothing. The machinery stays deliberately: a future open-but-unfeatured game joins the strip with no code change (the reappearance lock in `tests/test_docket_lounge_strip.py`). Its rules are unchanged when it renders: registry-generic (mark, name, tagline, static cadence copy from `lounge_cadence`, enrollment-aware outline CTA), Informational recipe, static by ruling (D21-eng), `.cta-outline` never `.cta-seal`.

**The headliner panel system** (`.hl-*`, ADR-049 — The Undercard). The multi-featured shell's vocabulary: `.hl-stage` (the wide composite container), `.hl-duo` (the paired grid whose `--paired` modifier draws the gold seam and centers the `.hl-seal`), `.hl-panel` (Ceremonial ground carrying the game's `--hl-wash`; the panel IS the card, so interior `.summons`/`.decree`/`.wl-card` chrome strips inside it), `.hl-conv` (the conversion-card variant of `.join`), `.hl-tape` (tale-of-the-tape stat lines), `.hl-board` / `.hl-tile` (a headliner's slot board as verdict tiles: accent-tinted when held, the live dot while a case plays, lit through the `--live-*` tokens with the letter inside once it is final; The Docket's, 2026-09-12), `.hl-record` (a record in the 2.7rem verdict register, wins and losses each in their live token), and `.hl-cta` (the accent-filled action, with an `--outline` variant). **ADR-052 (2026-08-19): the solid `.hl-cta` is the single action-button treatment across every panel state and both games — join, enter, choose, open your sheet — so gold never fills a lounge button; the accent `.hl-cta--outline` is reserved for a genuinely secondary, already-acted action (CFB's HELD "Review Pick"), and every pre panel opens with a matched ◇ `.summons-eyebrow--info` (the CFB decree's former `<h2>` folded into it). Locked by `tests/test_lounge_cta_parity.py`.** Panels take slug-suffixed modifiers (`.hl-panel--cfb`, `.join--docket`) that map onto the lounge accent tokens; adding a headliner costs one token pair and one mapping rule. Bone text throughout; `--hl-accent-bright` appears only at large-text sizes. Since the 2026-08-18 design review (ADR-051): composite order is greet → duo → ledger strip → second bill → tiles → commish note; panel headers bill the registry `lounge_label` full game names ("CFB Survivor", "The Docket · Pick 'Em"), and since 2026-09-25 the mark and name are one `.hl-panel-link` into the game's room (bone at rest, a large-text `--hl-accent-bright` chevron, the accent underline on hover and focus; every state shares `core/main/templates/main/_hl_panel_head.html`); every pre panel opens with the decree seal band (per-era numbering); and lounge roster counts are floor-gated (`ROSTER_COUNT_FLOOR`, shared by both lounge services) so a tiny pre-launch pool never reads dead.
...
- **Logged-in utility (`.auth-wrapper > .card.auth-card`)**: single bone card centered on the Tribunal Black backdrop, no brand panel. The Tribune voice carries through the eyebrow and copy inside the card. Used by `change_password.html` and `profile.html`. Both surfaces are reached through the navbar dropdown after login; the brand re-introduction would be redundant.
  - **`.auth-card--letter`** is the left-aligned variant of the utility card for pages made of steps, carrying the pre-ADR-066 Club Letter anatomy top to bottom (eyebrow → H1 → lede → steps/facts → one CTA → supporting line; `/app` meets the Eyebrow Rule when next reworked); profile and change-password keep the centered default. Used by `/app` (the installable-app page, The Wire). The eyebrow is Teko 500 `0.85rem`/`0.14em` lifted to `--gold-dark` on bone (Commish Gold itself is too light there, matching the auth links); the single CTA is the auth-page Trophy `.btn-primary`.
...
- **Current-user row** (`.row-current-user`): tint-only highlight — the platform fallback is a gold tint; game tables override the tint color only (CFB crimson, WC red — red per the WC accent rank; Docket garnet, which in that room means "yours" and never an outcome). Never a side-stripe (the last one, Golf's, was removed 2026-07-20). The highlight is the user's own line in the standings; it should be subtle, not loud.

### Per-game primitives

Games define their own primitives for surfaces and patterns specific to that game (tier indicators, ceremonial slots, sub-nav accents, etc.). See each game's `DESIGN.md` — World Cup at `games/worldcup/DESIGN.md` codifies the three-primitive tier trio (`.wc-tier-dot` / `.tier-badge` / `.wc-multiplier-chip`), the `.wc-stat-card` Casual-Light reference, the `.wc-champion-banner` ceremonial dark surface, and the `.wc-eyebrow` variants; The Docket at `games/docket/DESIGN.md` codifies the `.docket-side` pick primitive, the `.docket-ledger-*` season-surface family, and the `.docket-verdict-banner` ceremonial dark surface (gated on a completed season, so the light room stays light).
...
- **Eyebrow** — see §3 for the platform-foundation primitive. Quick reference:
    ...
    Reuse the matching primitive for a label that heads a value, list, paragraph or fold (game name, week, deadline, status), never one above a heading (The Eyebrow Rule, ADR-066). Don't invent a new "kicker" pattern.
...
- **Anatomy:** a 4px rule over the letter in the game's lounge accent (crimson `#C5050C`, garnet `#A63446`, Augusta `#006747`; Text Secondary `#5A5470` on club business) → Teko-stack H1 in Council Purple, the letter's first line, which names the game and week when the letter speaks for one ("The Docket, Week 3: 5-3", "CFB Survivor, Week 4: picks are open"; no eyebrow above it, ADR-066) → greeting only on personal mail → lede → the **fact block** (a bone inset ringed `1px #E8E5F0`, label above value, at most three facts, the deadline first; never a side-stripe, never the hero-metric trio) → extras the CTA acts on → **exactly one CTA** (solid game accent with bone text; club business wears flat Commish Gold with Chamber text; gold never fills a game button) → one supporting line → footnotes (the "Settle the tab" strip, a text link that names its game).
- **Copy:** no em dashes, no emoji, "CT" never "CDT", one deadline formatter (`utils.time.format_deadline_short`), subjects read `{What}: {Game}, Week {n}` within 45 characters, game headlines read `{Game}, Week {n}: {what}`, results in words (Survived / Lost a life) never colour, each letter states its consequence once, the tab strip names its game.

- **The announcement's blocks** (the Commish's markup, `utils/letter_markup.py`): section heading (the H1's Teko a step down, ink), label sub-head, bold/italic, Council Purple underlined links, lists (a numbered list keeps its first number), a pull quote in large italic Newsreader between two short gold rules with its credit as a small label (no dash rendered), a callout in the bone inset, a short centered gold divider, and big numbers stacked one per row in the bone inset, never side by side as a hero-metric trio. Live boards render as `result_block`s.
...
```
 ...
 rule       4px, Text Secondary #5A5470      (club business)
 H1         The Morning Line, Week 4         (the Paper's name and week)
 lede       Both boards are open. One sentence.   (the state opens it)
 ─────────────────────────────────────────── hairline
 section    CFB SURVIVOR · WEEK 4            (game accent)   <- sections in DEADLINE order
...
- **Blocks:** `game_section(slug, title, lines, deadline=, deadline_label=, button=, url=)` is one game's section, a mini-letter: hairline, the section head in the game's lounge accent (a label heading the section's lines, not an eyebrow over a heading), one paragraph per line, the one-row bone inset (the deadline through `format_deadline_short`), one solid game-accent button (`section-cta`, never `cta`). A spectator section (an eliminated Survivor member) passes no deadline and no button: section head and lines only. `rider_block(slug, sentence, link_label, url)` is the merged reminder's footnote: the tab strip's register, bold "Also on your desk.", the game's name, one sentence carrying that game's deadline, one accent text link, its only tap target.
- **Rules (locked in `render_letter`):** one section per game with something to say; one CTA per section and no club-gold button when any game button is present; sections in deadline order, undated sections last; `game_slug=None` chrome only when two or more sections render (a single-section Paper is that game's own letter: its headline, accent, subject). Subjects read `The Morning Line, Week {n}: {state}` within 50 characters, the Paper's one relaxation of the 45-character rule; the preheader carries the personal hook. One tab strip per owed game, in section order, after the sections.
...
#### The Tribune: the letter as page content

The third Club Letter shape (`core/tribune`; The Dateline Calendar, chosen 2026-09-24 on the decision page, seed `ccd3946d`): every sent `Announcement` is a back issue of the Commissioner's Club Tribune, read at `/tribune` (the calendar) and `/tribune/<id>` (one issue as printed). Members only, reached from the lounge line and the account dropdown; no comments, reactions or read state; the platform's light world with no new token.

- **The calendar:** Teko 700 "The Tribune" masthead and a one-line Newsreader lede in `--text-secondary` over a 2px gold rule, then the weeks, latest first, in a 40rem column. A week head (`.tribune-week-head`) is Teko 600 1.5rem ink, "Week N" plus that week's Saturday as its dateline (Teko 500 .95rem, `--text-secondary`, from the Docket week math), on a 2px Commish Gold rule; sticky under the navbar at `top: 58px`, static at 575.98px. An entry is one link and no card (`.tribune-entry-link`): gold "No. N" (`.tribune-no`, `--gold-dark`) and the send date in Teko, the Teko 600 1.7rem headline in Council Purple with a 2px gold underline on hover and focus, the Newsreader deck in `--text-secondary` at 62ch; one `--border-light` hairline between entries. Letters with no week file last under "Between seasons". Empty state: "Nothing on the record yet." over "The first letter goes out when the Commish has something to say. It will be kept here, under its week."
- **The issue as printed:** `.back-link` and the issue line ("No. N · Week N, Saturday, Sep 19") over `.tribune-sheet`, a white card with `--shadow-md`, 12px radius at every width, 44rem. Inside, `.letter-page` (`templates/letters/page.j2`, rendered only by `render_letter_page`) is the letter without its shell: 40rem measure, the Teko 600 headline first (no eyebrow, ADR-066; issues sent before ADR-066 shipped keep theirs, frozen in `sent_page_html`, which is why `.letter-page-eyebrow` stays in `style.css`), in Council Purple clamped 1.9 to 2.4rem, the letter's blocks untouched, the one CTA a Teko text link on a 2px gold underline (`.letter-page-cta a`), never a button. The foot says the audience and send date, then the week around it, always: "Also filed under Week N: …", "Filed under Week N, the only letter that week.", or "Filed between seasons." Under the sheet, the walk (`.tribune-walk`): "‹ Earlier · No. N" and "Later · No. N ›" with their headlines in purple; "The first issue" / "The latest issue" at the ends.
- **The lounge line** (`_tribune_line.html`, under From the Commish): `.home-shell .tribune-line*` only. A hairline above, no card: gold Teko "Latest letter", the headline as a bone italic Newsreader link with a bone-mute underline, the `--bone-mute` Teko date, "All issues ›" in gold pushed right. Headline and date are one unit (`.tribune-line-issue`); on phones All issues takes its own row. Absent when nothing has been sent. Lounge classes and tokens only (the accent firewall).
- **The filing rule.** A letter files under the "Week N" its headline or subject names, else the club week of its send (`filed_week`), else between seasons.
- **The frozen-at-send rule.** `sent_page_html` and `week_number` are stored when the desk sends, so a live board reads as it went out (`flask tribune backfill` fills older rows).

Locks: `tests/test_tribune.py`; `tests/test_email_letter.py`'s shell lock (`templates/email/` holds only `letter.j2`, so the page shell is `templates/letters/page.j2`); `tests/test_lounge_accent_firewall.py`; `tests/test_design_p2_s2_3_1.py`, which reads the file's first 575.98px block, so the lounge line's phone rule lives in the Tribune section's breakpoint block at the end of `style.css`, not the HOME section.
...
- **Do** carry the Eyebrow primitive (Teko 500, 0.85rem, 0.14em letter-spacing, gold, uppercase) over a value, list, paragraph or fold that needs a label; never above a headline (ADR-066).

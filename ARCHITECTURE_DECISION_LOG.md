...
| ADR-029 | Emoji avatar storage | File upload, Gravatar, Single emoji column | **Nullable String(4) on User model** | Emoji avatar stored as `avatar_emoji` column. Default is `⚽` via `get_avatar()`. No file storage overhead. All game standings must display avatar via `user.get_avatar()`. Required integration point for all future games. | 2026-04-12 | Yes |
| ADR-030 | Email sending consolidation | Per-game SMTP helpers, Shared utility | **Shared `utils/email.py` platform helper** | Fulfills ADR-016. Single `send_platform_email()` replaces three game-specific SMTP helpers (Golf `send_email()`, CFB `_send_email()`, CFB `send_admin_email()`). Consistent "The Commissioner's Club" From name on all outbound email. Game-specific content assembly stays in each game's reminders module. | 2026-04-12 | Yes |
